...
The empirical equivalence with smoothing means Tier 2 is a
\emph{cost-optimisation choice}, not a correctness one.  Justified
if (a) pedagogical use cases demand per-step cost lower than
smoothing affords, or (b) we want to extend the framework to
plasma-style scenarios where bump's per-particle cost still
competes with smoothing's per-step cost.

Decision deferred pending the orbital-test downstream verification:
run Stage 27 with both (bump $R=6$ via current incomplete-Tier-1
hybrid AND TSC + smoothing $N=16$) and confirm orbital drift is
acceptably small in both.  If yes, leave Tier 2 as future work.
If only one works for orbits, do Tier 2 for the winner.
