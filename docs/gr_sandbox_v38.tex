...
\fancyhead[R]{\small\color{gray}Design Document\,---\,v38}
...
\begin{titlepage}
  ...
  \vspace{1.2cm}
  {\large\bfseries\color{midblue} Revision v38\par}
  \vspace{0.2cm}
  {\small\color{gray}
  \textbf{v37 derives the GEMPIC canonical-momentum push step in
  detail, then -- via Fourier analysis and direct empirical comparison
  (Stage 41) -- reaches the conclusion that GEMPIC is NOT a
  structural fix for the Stage 37 uniform-motion heating.}  The
  analytic argument's load-bearing assumption (that HE matched-shape
  kills the discrete $\nabla\phi$ at a \emph{moving} particle) does
  not hold; HE only delivers zero gradient at $v=0$.  The structural
  problem lives in the deposit-and-gather pair, not the pusher.
  See the addendum §\ref{sec:gempic_addendum}.  GEMPIC is committed
  as a default-off diagnostic (\texttt{3e0b209}) -- the
  infrastructure is reusable if a future scheme combines it with a
  deposit-side construction that does target the actual structural
  property.
  Inherited from v36:
  \textbf{Stage 37 uniform-motion diagnosis exposes a structural defect
  in the discrete $-\nabla\phi - \partial_t\vv{A}$ cancellation on a
  moving deposit, and v36 specifies the benchmark suite and the
  canonical-momentum (GEMPIC) reformulation required to address it.}
  Prior orbit-test ``fixes'' (half-step $\vv{A}$ potentials, inductive
  sign flip) are re-labeled as partial mitigations of the orbit-level
  symptom rather than structural fixes.  See
  §\ref{sec:stage37_diagnosis} for the diagnosis, §\ref{sec:gempic_benchmark}
  for the benchmark scope, and §\ref{sec:gempic_design} for the
  canonical-momentum reformulation.\par
  \vspace{0.4em}
  \textbf{Inherited from v35:} implementation alignment with §\ref{sec:numerical}'s Yee
...
%====================================================================
\section*{Changes from v37 (this revision)}
%====================================================================

After v37's negative result on GEMPIC, the direction pivoted from
pusher rewrites to deposit-side mitigation: suppress the spurious
self-force on a moving deposit by making the macroparticle wider
or smoother.  v38 scopes a parameterizable kernel design that
addresses three concerns Dan articulated:

\begin{enumerate}
  \item Kernel size should be a runtime knob, not a code path per
    order.  B-splines $W_4, W_5, \ldots$ as separate integer orders
    each need their own Esirkepov implementation.  A parameterizable
    kernel with continuous scale $R$ gives one implementation,
    runtime-configurable.
  \item Truncated B-splines (and truncated Gaussians) are not the
    optimal compactly-supported kernels.  The variational result:
    minimising $\int k^2 |\hat W(k)|^2 \dd k$ subject to compact
    support and normalisation gives the lowest Dirichlet-Laplacian
    eigenmode of the support domain.  In 1D this is a half-cosine;
    in 2D circular it is the first lobe of $J_0$.
  \item The $C^\infty$ bump $\exp(-1/(1-x^2))$ has sub-exponential
    Fourier decay $\sim k^{-3/4}\exp(-c\sqrt{k})$ -- faster than
    any polynomial $1/k^n$.  Best tail-decay candidate at compact
    support.
\end{enumerate}

New section §\ref{sec:kernel_design} scopes the design space, the
implementation tiers, and the comparison plan.  Code in this
revision: \texttt{GR\_SHAPE\_BUMP} added behind shape-function
dispatch on the rho / phi-gradient-gather side of the HE adjoint;
J / Esirkepov stay on TSC for this first cut.  Stage 43 measures
whether kernel shape matters at fixed 3-cell support.  If yes,
follow-up work extends to parameterizable $R$ and bump-side
Esirkepov.

All v36 / v37 content carries forward unchanged.  The v37 addendum
§\ref{sec:gempic_addendum} (GEMPIC empirically disproven) stands;
v38 is the next attempt at the deposit-side mitigation it pointed to.

\newpage

%====================================================================
\section{Parameterizable Macroparticle Kernel}
\label{sec:kernel_design}
%====================================================================

\subsection{Motivation: deposit-side fix for Stage 37}

The Fourier analysis in v37 §\ref{sec:gempic_addendum} pinned the
Stage 37 spurious force to the discrete $\partial^h_y \phi$ at the
moving particle position.  In Fourier:
$$
\partial^h_y \phi(\vv x_p) \;\propto\; \int k_y \,\hat W(\vv k)\,
   \big[\hat G_h(\vv k, \vv k\cdot\vv v_0)
        - \hat G_h(\vv k, -\vv k\cdot\vv v_0)\big]\,\dd^2 k.
$$
The factor of $k_y$ shifts the integrand's dominance toward higher
$k$ than the field itself would (which is $\int \hat W \hat G$ with
no $k_y$).  Suppressing $\hat W$ at high $k$ is the relevant lever.

\subsection{Kernel candidates and their high-$k$ behaviour}
...
\textbf{Bump} $\exp(-1/(1-(x/R)^2))$ & $C^\infty$ & $k^{-3/4}\exp(-c\sqrt{k})$ \textbf{*}\\
Bessel $J_0(\alpha r/R)$ (2D, $r<R$) & $C^0$ & $1/k^{2}$ \\
\bottomrule
\end{tabular}
\end{center}

The variational optimum for $\min \int k^2 |\hat W|^2 \dd k$ on
compact support with Dirichlet $W(\pm R) = 0$ is the half-cosine in
1D (Dirichlet Laplacian ground state $W'' = -\lambda W$,
$W = \cos(\pi x/(2R))$, $\lambda = (\pi/(2R))^2$) and $J_0(\alpha r/R)$
on the 2D disk ($\alpha \approx 2.405$).  These minimise the
mean-squared frequency spread but have only $C^0$ extension at the
boundary, so their tail decay is only $1/k^2$.

The bump function has $C^\infty$ extension at the boundary, giving
sub-exponential decay -- faster than any polynomial.  Best
tail-decay candidate.

Which criterion matters for Stage 37 depends on where the asymmetric
part of $\hat G_h$ lives in $k$-space.  Worth testing empirically.

\subsection{Implementation tiers}

\textbf{Tier 1 (this revision, minimal viable test):}
add \texttt{GR\_SHAPE\_BUMP} as a new \texttt{shape\_function}
option, with bump weight + derivative evaluation, used in:
\begin{itemize}
  \item rho deposit on corners,
  \item LB-style $\nabla \phi$ gather (the spurious-force channel).
\end{itemize}
Keep TSC for J / Esirkepov and $\vv A$ interpolation.  This breaks
the rho-J shape symmetry but preserves HE adjoint on the
rho/$\phi$-gather pair, so Stage 39 (static reciprocity at $v=0$)
should still pass.  Stage 43 measures bump-vs-TSC on the phi-only
force.  Bump support: 3 cells (matched to TSC for first cut).

\textbf{Tier 2:} extend bump kernel to J Esirkepov so rho-J shape
is consistent.  Requires a parameterizable Esirkepov over a
variable-width cell window.  ~200 lines plus tests.

\textbf{Tier 3:} introduce continuous scale parameter $R$
(\texttt{gr\_sim\_set\_kernel\_radius}).  Cell-window size adapts
to $R$; auto-scale $R \propto 1/\dd x$ for fixed physical width.

\textbf{Tier 4:} add comparison kernels (half-cosine, Bessel $J_0$
first lobe) as additional \texttt{shape\_function} options.
Empirical comparison of all options.

\textbf{Tier 5:} if any prior tier validates the principle,
promote the chosen kernel + auto-scaled $R$ to production default.
Mark Boris-pusher and TSC path as legacy.

\subsection{Tier 1: bump weight on 3-cell support}

Map the natural support of $\exp(-1/(1-u^2))$ ($u \in [-1, 1]$) onto
cell-distance range $[-1.5, 1.5]$ to match TSC.  Define
$$
b(d) = \frac{1}{N_b}\,\exp\!\bigg(\!\!-\frac{1}{1 - (d/1.5)^2}\bigg)
\quad\text{for } |d| < 1.5,
$$
zero otherwise.  $N_b$ is the normalisation
$\int_{-1.5}^{1.5} \exp(-1/(1-(d/1.5)^2)) \dd d \approx 0.6660$ so
that $\int b(d) \dd d = 1$.
...
$$
Both $b$ and $b'$ go smoothly to zero at $d = \pm 1.5$ in all
derivatives ($C^\infty$ extension), and $b'(0) = 0$ by symmetry.

\subsubsection*{Cell-sum normalisation}

For a particle at sub-cell offset $u \in [-0.5, +0.5]$ from its
nearest cell, weights at cell offsets $\{-1, 0, +1\}$ are
$w_i = b(i - u)$, normalised so $\sum w_i = 1$ at every $u$.  Note:
the function $b$ is normalised so its INTEGRAL is 1, but the
discrete SUM at cell positions varies with $u$.  In practice we
either (a) renormalise the discrete weights to sum to 1 (preserves
charge conservation but changes the effective kernel shape away
from pure bump) or (b) accept O$(10^{-2})$ charge non-conservation
per particle and document.

Recommended: option (a) -- renormalise.  The shape on the grid is
then "discretised bump" which loses some of the bump's $C^\infty$
boundary smoothness but keeps the right qualitative high-$k$
behaviour.

\subsubsection*{Stage 43 acceptance}

Compare TSC vs bump on Stage 37's v-scan at \texttt{mass=1.0,
rho\_smooth=0}.  For each $v$, report:
\begin{itemize}
  \item \texttt{accel(phi-only, TSC)},
  \item \texttt{accel(phi-only, BUMP)},
  \item \texttt{ratio = accel(BUMP) / accel(TSC)}.
\end{itemize}
Also verify Stage 39 reciprocity for BUMP (should pass at $v=0$).

\textbf{Threshold for "kernel shape matters at fixed support":}
\texttt{|ratio| < 0.5} at at least half the $v$ values.  If this
holds, Tier 2 / 3 are worth pursuing.  If \texttt{|ratio|} stays
near 1 at every $v$, kernel shape doesn't matter at fixed support
and we move to widening (Tier 3) instead of shape-comparison.

\textbf{Threshold for "Tier 1 should not regress anything":}
Stage 39 reciprocity ratio still $< 10^{-6}$ at every separation
(BUMP preserves HE adjoint on the rho/$\phi$-gather pair).

\subsection{Empirical findings (2026-05-25): equal-distance methodology}
\label{sec:kernel_v38_findings}

The Tier 1 implementation (commit 425ed05) was extended to a
\emph{parameterized} bump kernel with continuous radius $R$ in cell
units (commit on this revision).  The radius controls a variable
cell window: 2*ceil($R$+0.5)+1 cells per axis, capped at 17 (R$=$8).
API: \texttt{gr\_sim\_set\_kernel\_radius(sim, R)} -- default $1.5$
matches TSC's 3-cell support.

\subsubsection*{Measurement methodology correction}

Initial measurements (Stage 41, Stage 43 v1) used a fixed 200-step
force-measurement window regardless of $v$.  This meant at low $v$
the particle moved $<1$ cell during the window, and the measured
force reflected the spurious force at a single (nearly fixed)
sub-cell offset rather than averaging over the full sub-cell
distribution.  This produced a spurious ``v=0.005 spike'' that was
purely a methodology artifact.

Stage 43 v2 (equal-distance) fixes this: window steps scale as
$5/(v\,\dd t)$ so the particle traverses 5 cells at every $v$,
sampling the full sub-cell-offset distribution.  Heavy particle
mass ($m=100$) keeps the spurious force from changing $v$
materially over the longer windows.

\subsubsection*{TSC baseline -- corrected absolute spurious force}

\begin{center}
\begin{tabular}{rrr}
\toprule
$v/c$ & accel (TSC bare, cells/$\dd t^2$) & n\_window \\
\midrule
$0.001$ & $-1.12\times10^{-8}$ & $7071$ \\
$0.005$ & $-7.40\times10^{-9}$ & $1414$ \\
$0.010$ & $-4.67\times10^{-9}$ & $707$  \\
$0.050$ & $-2.37\times10^{-9}$ & $200$  \\
$0.100$ & $+1.42\times10^{-9}$ & $200$  \\
$0.200$ & $+2.11\times10^{-10}$ & $200$ \\
$0.300$ & $+4.43\times10^{-9}$ & $200$  \\
$0.500$ & $+3.20\times10^{-8}$ & $200$  \\
\bottomrule
\end{tabular}
\end{center}

The absolute spurious force is $\sim 10^{-9}$ to $10^{-8}$ across
the whole $v$ range, much smaller than the $\sim 10^{-6}$ to
$10^{-5}$ numbers reported under the old methodology.  The
nonrelativistic regime IS stable in absolute terms; previous
characterisations were contaminated by sub-cell-offset bias in
the fixed-window measurement.

\subsubsection*{Bump-R vs TSC + smoothing: equivalent effectiveness}

Ratio table ($|$accel$/$accel(TSC)$|$, equal-distance methodology):
...
$0.050$ & $0.33$ & $0.06$ & $0.02$ & $\mathbf{0}$ & $0.04$ & $\mathbf{0}$ & $\mathbf{0}$ \\
$0.100$ & $2.93$ & $0.37$ & $0.07$ & $\mathbf{0}$ & $0.30$ & $\mathbf{0}$ & $\mathbf{0}$ \\
$0.200$ & $14.0$ & $8.0 $ & $\mathbf{0}$ & $\mathbf{0}$ & $\mathbf{0}$ & $\mathbf{0}$ & $\mathbf{0}$ \\
$0.300$ & $0.19$ & $0.33$ & $0.10$ & $\mathbf{0}$ & $\mathbf{0}$ & $\mathbf{0}$ & $\mathbf{0}$ \\
$0.500$ & $0.24$ & $0.11$ & $\mathbf{0}$ & $\mathbf{0}$ & $\mathbf{0}$ & $\mathbf{0}$ & $\mathbf{0}$ \\
\bottomrule
\end{tabular}
\end{center}

$\mathbf{0}$ entries indicate the spurious force has dropped below
float32 noise floor.

\subsubsection*{What this tells us}

\begin{enumerate}
  \item TSC's bare spurious force is already $\sim 10^{-8}$ at
    typical pedagogical $v$, much smaller than previous methodology
    suggested.  The artifact is not as severe as we'd been treating
    it -- it was inflated by measurement noise.
  \item For $v \geq 0.05$, both bump-$R$ at $R \geq 6$ and TSC +
    smoothing at $N \geq 16$ suppress the spurious force below
    float32 noise floor.  Effectively equivalent in result.
  \item For $v \leq 0.01$, neither approach helps because the TSC
    artifact is already at the noise floor at low $v$.  Nothing
    further to suppress.
  \item Choice between bump-$R$ and smoothing-$N$ is now about
    \emph{implementation cost}, not effectiveness:
    \begin{itemize}
      \item Smoothing: per-step grid convolution, scales with grid
        size $W \cdot H$, independent of particle count.  At $N=16$,
        roughly quadruples FDTD step cost.  Affects $\rho$ and $\vec J$
        consistently.
      \item Bump-$R$: per-particle deposit only, scales as
        $N_{\mathrm{particles}} \cdot R^2$, independent of grid size.
        For pedagogical few-particle sims at $R=6$ ($\sim 169$ cells
        per particle), cost is negligible.  Currently only affects
        $\rho$ deposit and $\phi$ gather; full coverage needs Tier 2.
    \end{itemize}
  \item For real-time pedagogical demos (few particles, target 60 fps),
    bump-$R$ is the better choice on cost grounds.  For plasma or
    high-particle-count regimes, smoothing wins.
\end{enumerate}

\subsubsection*{Implementation status and Tier 2 decision}

Bump-$R$ implementation in tree (commits 425ed05 + this revision):
\begin{itemize}
  \item \texttt{GR\_SHAPE\_BUMP} enum, \texttt{gr\_sim\_set\_kernel\_radius}.
  \item Variable-width $\rho$ deposit on corners, parameterised by $R$.
  \item Variable-width $\phi$-gradient LB gather on corners.
  \item Stage 39 reciprocity passes (ratio $= 0$ to float precision).
\end{itemize}

NOT yet in tree:
\begin{itemize}
  \item Bump Esirkepov for $\vec J$ on edge sublattices.
  \item Bump interp on X-edge / Y-edge for $\vec A$ reads.
  \item Bump LB gradient gathers on edge sublattices
    (\texttt{gr\_bump\_lb\_dx\_xedge} etc.).
\end{itemize}

For phi-only Stage 37 tests the current implementation is complete
and sufficient.  For \emph{full-force} EM PIC orbit tests (Stage 27
type) with the bump kernel, Tier 2 is required to maintain shape
consistency between $\rho$ and $\vec J$ wakes.

\subsubsection*{Tier 2 LANDED (commit 98b4fe9): bump Esirkepov + edge interp}

Tier 2 implementation:
\begin{itemize}
  \item \texttt{gr\_bump\_interp\_xedge} / \texttt{\_yedge}: variable-
    window bump interp on edge sublattices.  Used by
    \texttt{dt\_A\_em\_at\_total} when shape=BUMP.
  \item \texttt{gr\_bump\_esirkepov\_deposit\_jxy}: variable-window
    Esirkepov current decomposition with renormalised bump weights.
    Same separable algebra as existing CIC Esirkepov.
  \item Dispatch in \texttt{sim.c} (Esirkepov path) and
    \texttt{particle.c} ($\vec A$ edge interp).
\end{itemize}

Validation -- new Stage 44:
\begin{itemize}
  \item Part A: bump Esirkepov continuity verified at 5 values of
    $R$ x 5 trajectory shapes = 25 cases.  All max$|$residual$|$
    $\sim 10^{-8}$ (float32 noise).  Discrete continuity
    $\partial_t\rho + \nabla\cdot\vec J = 0$ holds at every corner.
  \item Part B: full force (inductive ON), bump R=8 and TSC + N=16
    smoothing are EMPIRICALLY EQUIVALENT.  Both drop spurious accel
    below float32 noise floor at $v \geq 0.05$.  Confirms the
    deposit-side mitigation works equivalently in either form.
\end{itemize}

\subsubsection*{Production recommendation (v38, post-Tier-2)}
...
Cost: O($N_\mathrm{particles} \cdot (2 \mathrm{hw}+1)^2$) per step for
the deposit; negligible vs FDTD for few-particle pedagogical sims.

For high-particle-count plasma simulations: switch to TSC + smoothing
\texttt{rho\_smooth\_passes = 16} (or higher), since per-step
convolution cost is constant in particle count.

Both paths are now production-ready and validated.

\subsubsection*{Tier-2 gap closure (commit e571c54)}

After the initial Tier-2 commit, an audit revealed three partial-
coverage gaps:

\begin{enumerate}
  \item \texttt{B\_em\_z\_at\_total} and \texttt{B\_g\_z\_at\_total}
    (curl $\vv A$ for the $v\times \vv B$ Lorentz force) used a
    centered-difference + CIC interp gather (\texttt{curl\_Agz\_sampled\_at})
    with no shape dispatch.
  \item \texttt{grav\_grad\_at\_lb} (gravity-side $\nabla\Phi_g$
    gather) had TSC and CIC branches but no BUMP.
  \item \texttt{dt\_A\_g\_at\_total} (gravity-side $\partial_t\vv A_g$
    edge interp) had only CIC.
\end{enumerate}

All three closed by:
\begin{itemize}
  \item Four new bump-LB mixed-axis edge gathers in deposit.c:
    \texttt{gr\_bump\_lb\_dx\_xedge}, \texttt{\_dy\_xedge},
    \texttt{\_dx\_yedge}, \texttt{\_dy\_yedge}.
  \item New \texttt{bump\_curl\_Az\_at} helper in particle.c that
    computes $B_z = \partial_x A_y - \partial_y A_x$ via the bump-LB
    mixed gathers.  Used by both \texttt{B\_em\_z\_at\_total} and
    \texttt{B\_g\_z\_at\_total} when shape=BUMP.
  \item BUMP branches added to \texttt{grav\_grad\_at\_lb} and
    \texttt{dt\_A\_g\_at\_total}, parallel to the existing EM-side
    BUMP branches.
\end{itemize}
...
Bump $R=8$ now slightly beats TSC $+$ $N=16$ on both sectors -- the
HE adjoint pair is matched end-to-end and the bump approach is the
better choice in addition to being cheaper per step.

\subsubsection*{Convenience API: \texttt{gr\_sim\_use\_pedagogical\_defaults}}

After the cleanup of debugging knobs (commits 5735ef7, d4f1f5a), a
single convenience function applies the recommended pedagogical
...
The bundle sets: \texttt{shape\_function = GR\_SHAPE\_BUMP},
\texttt{kernel\_radius = 6.0}, \texttt{force\_interp = LEWIS\_BIRDSALL},
\texttt{particle\_source\_deposition = 1}, \texttt{damping = 16}.
Tests that need to exercise a specific non-default knob (Stage 11
for CIC Esirkepov continuity, Stage 18 for LB regression, etc.)
should NOT call this and instead set required knobs explicitly.

\texttt{gr\_sim\_create} defaults are NOT changed; this is a
strictly additive convenience.  Existing tests' calibrations hold.
Migration to the bundle is incremental and per-test as appropriate.

\subsubsection*{Stage 47: equal-coupling EM binary -- bump kernel still works}

Stages 45 (EM Kepler) and 46 (GR Kepler) use a \emph{test particle}
configuration: $q_{\mathrm{test}}/Q_{\mathrm{central}} = 10^{-3}$ or
$m_{\mathrm{test}}/M_{\mathrm{central}} = 10^{-3}$.  Stage 47 is the
analog with BOTH bodies as full PIC sources at equal coupling:
$m = Q = 0.01$, opposite charges.  In this regime self/mutual force
ratio is 1 -- the self-force per particle is comparable to the
mutual force.

\emph{Methodology caveat from initial run (corrected below):}  The
first Stage 47 attempt did not explicitly disable gravity ($G_{\mathrm{eff}}=1$
at default).  With $m = Q$ and $G_{\mathrm{eff}} = k_e$, 2D-log
gravitational attraction is COMPARABLE to EM attraction at the same
parameters, so the orbital dynamics were actually mixed EM + gravity,
not pure EM.  Mixed dynamics produced large chaotic drift ($-60\%$ to
$-90\%$ over 4 orbits) that misled the initial conclusion.

\emph{Corrected setup} ($G_{\mathrm{eff}} = 0$, gravitomagnetic disabled):
...
\midrule
TSC bare      & \textbf{UNBOUND} (only 1/4 orbits completed) \\
...
\textbf{Bump $R=8$ delivers $+5.3\%$ drift -- comparable to Stage 45/46's
test-particle results.}  Smoothing $N=16$ gives essentially the same
behavior.  TSC bare is fully unbound after 1 orbit; the equal-coupling
self-force without mitigation destroys the orbit immediately.

The marginal BUMP $R=6$ result deserves note: at $r_{\mathrm{orb}}=20$,
the kernel width $2\,\mathrm{ceil}(R+0.5)+1 = 15$ cells.  Two particles
on opposite sides of the orbit have separation $2r = 40$ cells.  Kernel
overlap is $2\cdot 7 < 40$, so no real-space overlap.  But the long
tails of the bump deposits may overlap subtly in ways that perturb the
orbit; $R=8$ (17-cell width) appears more robust here.  Could be
investigated further but isn't urgent for production.

\textbf{Pedagogical scope implication:} the v38 production setup
(bump kernel, $R = 6$--$8$) is reliable for BOTH:
\begin{itemize}
  \item Test particle around analytic background (Stages 45/46, $\sim 2$--$3\%$ inspiral)
  \item Equal-coupling fully-PIC binaries (Stage 47, $\sim 5\%$ drift at $R=8$, EM-only)
\end{itemize}
Both demos are stable and quantitatively interpretable.

\subsubsection*{Stage 48: Bertrand-aware conservation diagnostic}

Dan, 2026-05-26: ``With a 1/r force law, the interaction is strong,
there may not be orbits that are closed (Bertrand's theorem), they
may oscillate or precess.  There are a number of things to check:
that the absorbing boundary conditions are doing their job,
momentum/energy conservation of the two bodies, and what happens in
the limit of nonrelativistic velocities.''

Bertrand's theorem says only $1/r^2$ (3D Kepler) and $r^2$ (harmonic)
central forces give closed orbits.  GRlite's 2D log Coulomb gives
force $\propto 1/d$, which is NOT Bertrand: any deviation from
exactly circular initial condition gives a non-closed rosette with
radial oscillation between $r_{\min}$ and $r_{\max}$ while $\theta$
advances.  This means Stage 47's ``\% drift in radius at each
$\theta$-wrap'' metric conflates physical rosette behavior with
numerical heating.

Stage 48 replaces that with CONSERVATION LAWS, which must hold
regardless of orbit shape:
\begin{itemize}
  \item $E_{\mathrm{tot}} = E_{\mathrm{kin}} + U_{\mathrm{int}}$ with
    $U_{\mathrm{int}}(d) = +2 k_e Q^2 \ln(d)$ (attractive 2D Coulomb)
  \item $L_z$ (total) about COM
  \item $P_x, P_y$ (linear momentum, $\sim 0$ by symmetry)
  \item max $|\phi_{\mathrm{em}}|$ at PML inner edge vs.\ bulk
\end{itemize}

Three configurations probe Dan's concerns:

\paragraph{(A) $m{=}0.01$, $v_{\mathrm{factor}}{=}1$ ($v_{\mathrm{orb}}{=}0.1$, mildly rel.\ circular):}
TSC bare chaotic; TSC$+$N=16, BUMP $R{=}6$, BUMP $R{=}8$ all preserve
$E$ to $\pm 3\%$ and $L_z$ to $\pm 15$--$21\%$ over 4 orbits.
Separation oscillates $\pm 10\%$ -- this is the physical rosette,
not numerical drift, since $E$ is well-conserved.  PML edge/bulk
field ratio $\sim 10\%$.

\paragraph{(B) $m{=}0.01$, $v_{\mathrm{factor}}{=}0.1$ ($v_{\mathrm{orb}}{=}0.01$, low-L NOT circular):}
This is NOT a non-relativistic test -- it sets $L_z$ to $1/10$ the
circular value at $d{=}40$.  In the 2D-log effective potential
$V_{\mathrm{eff}}(d) = L^2/(2 m_{\mathrm{red}} d^2) + 2 k_e Q^2 \ln(d)$,
the inner turning point sits at $d_{\min} \approx 1.55$ cells --
below all kernel scales.  The orbit physically dives toward
$d_{\min}$.  At close approach, deposit/gather kernels overlap and
PIC heating dominates; particles get kicked apart.  All four configs
fail catastrophically (separation grows $2000$--$8000\%$, $P_x$
drifts to $5 \times 10^{-3}$).  PML ratio $24$--$44\%$ (worse static-
field absorption at low $v$).  Not a kernel-mitigation failure -- a
fundamental limit of PIC at sub-kernel close approach.

\paragraph{(C) $m{=}1.0$, $v_{\mathrm{factor}}{=}1$ ($v_{\mathrm{orb}}{=}0.01$, PROPER non-rel circular):}
True non-relativistic limit with circular IC.  Heavier particles
give $v_{\mathrm{orb}} = Q\sqrt{k_e/m} = 0.01$ at $d{=}40$, and
$V_{\mathrm{eff}}$ has its minimum at $d{=}40$ -- so any perturbation
gives only small rosette amplitude.  Results over 4 orbits:

\begin{center}
\begin{tabular}{lcccc}
\toprule
config & $\Delta E / |E_0|$ & $\Delta L_z / |L_0|$ & $|d - d_0|/d_0$ & $|P_x|$ max \\
\midrule
TSC bare        & $\pm 2\%$        & $-198\%$ (reversed!) & $0$--$+68\%$ & $5 \times 10^{-4}$ \\
TSC $+$ N=16    & $\pm 0.45\%$     & $\pm 0.8\%$          & $\pm 13\%$   & $1 \times 10^{-7}$ \\
BUMP $R{=}6$    & $\pm 0.37\%$     & $\pm 1.3\%$          & $\pm 12\%$   & $2 \times 10^{-4}$ \\
BUMP $R{=}8$    & $\pm 0.35\%$     & $\pm 1.1\%$          & $\pm 13\%$   & $1 \times 10^{-4}$ \\
\bottomrule
\end{tabular}
\end{center}

Mitigation configs preserve $E$ to better than $0.5\%$ and $L_z$ to
better than $1.5\%$ in proper non-rel circular.  TSC$+$N=16 has
remarkable momentum conservation ($10^{-7}$); the bump kernels give
$\sim 10^{-4}$ which is small but suggests a sub-cell asymmetry in
the bump weight evaluation worth probing.

\paragraph{Answers to Dan's three checks:}
\begin{itemize}
  \item \textbf{PML doing job}: yes, $\sim 10\%$ absorbing ratio at
    circular orbits.  Worse ($24$--$44\%$) when orbit collapses to
    small $d$, but that's a different failure mode (close-approach
    near-field swamps).
  \item \textbf{Conservation of $E$ and $L_z$}: $<\!0.5\%$ in proper
    non-rel circular for all mitigation configs.  $\sim 3\%$ for $E$
    and $\sim 15\%$ for $L_z$ at $v/c = 0.1$ (Case A); plausibly
    relativistic corrections to $L_z$ definition (we used canonical
    angular momentum which mixes mechanical and field momentum at
    higher $v/c$).
  \item \textbf{Non-rel limit}: clean.  Case C shows the v38
    production setup works correctly at $v/c = 10^{-2}$.
\end{itemize}

\paragraph{Stage 47 \% drift retraction (further):}
The $\sim 5\%$ ``drift'' reported in Stage 47 BUMP $R=8$ over 4
$\theta$-wraps was mostly the 2D-log rosette physics, not
numerical error.  Real numerical drift (from $E$ conservation) is
$<\!5\%$ at $v_{\mathrm{orb}} = 0.1$ and $<\!0.5\%$ at
$v_{\mathrm{orb}} = 0.01$.  The bump kernel is performing better
than the radius-drift metric implied.

\newpage

%====================================================================
\section*{Changes from v36 (the v37 revision)}
%====================================================================

This revision works out the GEMPIC canonical-momentum time-stepping
flagged as "to be locked via literature check" in v36
§\ref{sec:gempic_design}.  Three concrete additions:

\begin{enumerate}
  \item A clean Hamilton's-equations derivation of the canonical
    EOMs, including the algebraic identity that converts
    $q\nabla(\vv{v}\cdot\vv{A})$ in the canonical force law to the
    Lorentz-form $-q\partial_t\vv{A} + q\vv{v}\times\vv{B}$ at the
    continuum level.  Both forms are exact; GEMPIC uses the first
    because it requires only scalar-gradient discrete operators
    that the Yee staggering naturally provides.
  \item A concrete \emph{relativistic leapfrog} scheme for the
    canonical push, with explicit time-staggering for $p_c$, $x$,
    and the field reads.  Treats the implicit dependence of
    $\vv{v}$ on $p_c$ at the relativistic level via a single
    fixed-point step that we show converges in O(few) iterations
    for $v < 0.9c$ and is exact for non-relativistic $v$.
  \item An \emph{analytic verification} that the proposed discrete
    force law gives identically zero net force on a uniformly-moving
    point deposit, in the limit of a static field source and with
    matched TSC deposit / TSC-derivative force gather.  This is the
    structural guarantee the Stage 37 diagnosis showed is absent
    from the current $-\nabla\phi - \partial_t\vv{A}$ pairing.
\end{enumerate}

All v36 content (Stage 37 diagnosis, benchmark suite scope, GEMPIC
high-level scope) carries forward unchanged.

\textbf{Important update (2026-05-24, addendum
§\ref{sec:gempic_addendum}):} after writing the derivation above,
the load-bearing claim of the analytic argument was checked via
direct Fourier analysis of the discrete Yee operator and via an
empirical comparison (Stage 41) of Boris vs GEMPIC pushers on the
Stage 37 setup.  Both checks agree: \textbf{GEMPIC is not a
structural fix.}  At every $v$ in the Stage 37 sweep, the GEMPIC
spurious acceleration is within a few percent of the Boris value.
The structural problem lives in the discrete deposit's $\nabla^h\phi$
at the moving particle position (not in the pusher form), and
neither pusher addresses it.

GEMPIC remains in the tree as a default-off diagnostic (commit
\texttt{3e0b209}); the next scope item is a deposit-side
construction that targets the actual structural property at fault.
The original derivation in §\ref{sec:gempic_derivation} below is
preserved verbatim for the historical record; the addendum
§\ref{sec:gempic_addendum} corrects its load-bearing claim.

\newpage

%====================================================================
\section{GEMPIC Time-Stepping: Derivation and Discrete Scheme}
\label{sec:gempic_derivation}
%====================================================================

\subsection{Recap of the structural problem (v36 abridged)}

Stage 37 (\S\ref{sec:stage37_diagnosis}) showed that the discrete
operator pair $-\nabla_h\phi$ and $-\partial^h_t\vv{A}$ does not
constitute a discrete adjoint cancellation for a moving deposit:
the residual force is UV-divergent in $\Delta x$, dt-independent,
and grows worst at low $v$.  The current Boris pusher tracks the
\emph{kinetic} momentum $\vv{p}_{\mathrm{kin}} = \gamma m \vv{v}$
and computes $\dd \vv{p}_{\mathrm{kin}}/\dd t$ via the Lorentz
force law, which puts the broken $-\partial_t\vv{A}$ operator
directly in the push.  GEMPIC removes that operator by tracking
canonical momentum instead.

\subsection{Continuum derivation: Hamilton's equations}

A relativistic charged particle in an external electromagnetic
field has the canonical Hamiltonian
\begin{eqbox}
\begin{equation}
\label{eq:gempic_H}
H(\vv{x}, \vv{p}_c; t)
   = c\,\sqrt{m^2 c^2 + |\vv{p}_c - q\vv{A}(\vv{x}, t)|^2}
   \;+\; q\,\phi(\vv{x}, t),
\end{equation}
\end{eqbox}
where $\vv{p}_c$ is the canonical momentum, related to the
kinetic momentum by
\begin{equation}
\label{eq:gempic_pc_pkin}
\vv{p}_c \;=\; \vv{p}_{\mathrm{kin}} + q\vv{A}(\vv{x}, t)
       \;=\; \gamma m \vv{v} + q\vv{A}(\vv{x}, t).
\end{equation}

Hamilton's equations are
\begin{align}
\dot{\vv{x}} &\;=\; \frac{\partial H}{\partial \vv{p}_c}
              \;=\; \frac{c^2 (\vv{p}_c - q\vv{A})}{
                       \sqrt{m^2 c^4 + c^2|\vv{p}_c - q\vv{A}|^2}}
              \;=\; \vv{v},
              \label{eq:gempic_xdot}\\
\dot{\vv{p}_c} &\;=\; -\frac{\partial H}{\partial \vv{x}}.
              \label{eq:gempic_pcdot_abs}
\end{align}
Computing $\partial H/\partial x_i$ at fixed $\vv{p}_c$:
\begin{align}
\frac{\partial H}{\partial x_i}
  &= \frac{c^2 (p_{c,j} - qA_j)}{H_{\mathrm{kin}}} \cdot
     (-q)\,\frac{\partial A_j}{\partial x_i}
     + q\,\frac{\partial \phi}{\partial x_i}\notag\\
  &= -q\, v_j \, \frac{\partial A_j}{\partial x_i}
     + q\,\frac{\partial \phi}{\partial x_i},
\end{align}
where $H_{\mathrm{kin}} = c\sqrt{m^2 c^2 + |\vv{p}_c - q\vv{A}|^2}
                       = \gamma m c^2$.  Therefore
\begin{eqbox}
\begin{equation}
\label{eq:gempic_pcdot}
\dot{p}_{c,i} \;=\; q\, v_j \, \frac{\partial A_j}{\partial x_i}
                  \;-\; q\,\frac{\partial \phi}{\partial x_i}.
\end{equation}
\end{eqbox}
The sum convention is Einstein.  The term $q v_j (\partial A_j/\partial x_i)$
is $q\,\nabla_i(\vv{v}\cdot\vv{A})$ in the convention where $\vv{v}$
is held constant during the gradient (which it is in the Hamiltonian
derivation, since the partial derivative is at fixed $\vv{p}_c$ and
hence at fixed $\vv{v}$ except through $\vv{A}(\vv{x})$).

\subsection{Equivalence to the Lorentz form (continuum identity)}

Eq.~\eqref{eq:gempic_pcdot} is on the canonical momentum.  The
relation to the kinetic momentum, from
Eq.~\eqref{eq:gempic_pc_pkin}, is
\begin{equation}
\dot{\vv{p}}_{\mathrm{kin}}
   = \dot{\vv{p}}_c - q\,\dot{\vv{A}}(\vv{x}(t),t)
   = \dot{\vv{p}}_c - q\,\partial_t\vv{A}
                   - q\,(\vv{v}\cdot\nabla)\vv{A}.
\end{equation}
Substituting Eq.~\eqref{eq:gempic_pcdot} and the vector identity
\begin{equation}
\label{eq:vec_id}
\nabla(\vv{v}\cdot\vv{A}) - (\vv{v}\cdot\nabla)\vv{A}
   = \vv{v}\times(\nabla\times\vv{A}),
\end{equation}
where $\nabla(\vv{v}\cdot\vv{A})$ in this identity has the same
``$\vv{v}$ constant'' convention as
Eq.~\eqref{eq:gempic_pcdot}, gives the standard Lorentz form:
\begin{equation}
\dot{\vv{p}}_{\mathrm{kin}}
   = -q\nabla\phi - q\partial_t\vv{A} + q\vv{v}\times\vv{B}
   \quad\text{with}\quad
   \vv{B} = \nabla\times\vv{A}.
\end{equation}

So at the continuum level, the canonical
Eq.~\eqref{eq:gempic_pcdot} and the kinetic Lorentz form are
algebraically equivalent.  They differ in \emph{which discrete
operators} appear when we discretise.  GEMPIC uses
Eq.~\eqref{eq:gempic_pcdot} because it requires only scalar-gradient
operations on $\phi$ and $\vv{A}$ at the particle position; no
discrete $\partial_t\vv{A}$ and no discrete $\nabla\times\vv{A}$.
The discrete $\partial_t\vv{A}$ is the operator the Stage 37
diagnosis showed does not cancel against the discrete $\nabla\phi$
on a moving deposit, and is what is structurally absent from the
GEMPIC force law.

\subsection{2D specialisation of the force law}

In 2D ($\vv{x} = (x, y)$, $\vv{A} = (A_x, A_y)$, no $z$-dependence)
the GEMPIC force has two components:
\begin{eqbox}
\begin{align}
F_x \;=\; \dot{p}_{c,x} &\;=\; q\,v_x\,\frac{\partial A_x}{\partial x}
                    + q\,v_y\,\frac{\partial A_y}{\partial x}
                    - q\,\frac{\partial \phi}{\partial x},
                    \label{eq:gempic_Fx}\\
F_y \;=\; \dot{p}_{c,y} &\;=\; q\,v_x\,\frac{\partial A_x}{\partial y}
                    + q\,v_y\,\frac{\partial A_y}{\partial y}
                    - q\,\frac{\partial \phi}{\partial y}.
                    \label{eq:gempic_Fy}
\end{align}
\end{eqbox}
The four mixed-component derivatives $\partial A_i / \partial x_j$
($i, j \in \{x, y\}$) are evaluated at the particle position by
shape-function-weighted gathers from the Yee-edge sublattices on
which $A_x$ (X-edge) and $A_y$ (Y-edge) live.

\subsection{Discrete operators -- what we have, what we need}

The current code (post-Stage 37 additions) already implements:
\begin{itemize}
  \item Corner-stored $\phi$ with TSC value interpolation
    \texttt{gr\_tsc\_interp\_corner} and LB-style $\partial_x \phi$,
    $\partial_y \phi$ gathers
    (\texttt{phi\_em\_grad\_at\_lb} in \texttt{core/src/particle.c}).
  \item X-edge stored $A_x$ with TSC value interpolation
    \texttt{gr\_tsc\_interp\_xedge} and LB-style $\partial_x A_x$
    gather \texttt{gr\_tsc\_lb\_dx\_xedge}.
  \item Y-edge stored $A_y$ with TSC value interpolation
    \texttt{gr\_tsc\_interp\_yedge} and LB-style $\partial_y A_y$
    gather \texttt{gr\_tsc\_lb\_dy\_yedge}.
\end{itemize}
What GEMPIC additionally needs (the "mixed" gradients):
\begin{itemize}
  \item $\partial_y A_x$ at the particle position: gather from
    X-edge with a kernel that is the y-derivative of TSC.
    New: \texttt{gr\_tsc\_lb\_dy\_xedge}.
  \item $\partial_x A_y$ at the particle position: gather from
    Y-edge with a kernel that is the x-derivative of TSC.
    New: \texttt{gr\_tsc\_lb\_dx\_yedge}.
\end{itemize}
Both new gathers reuse the existing $dW_3/du$ kernel
(\texttt{tsc\_dw\_1d} in \texttt{core/src/deposit.c}); only the
sublattice indexing changes.  Estimate: ~50 lines each plus tests.

\subsection{Discrete time-stepping: leapfrog on the canonical state}

The state we evolve is $(\vv{x}^n, \vv{p}_c^{n-1/2})$ at integer
spatial step $n$ and half-step canonical momentum, paired with the
existing field state $(A_{\mathrm{prev}} = \vv{A}^{n-1/2},
A_{\mathrm{curr}} = \vv{A}^{n+1/2}, \phi^n)$ -- the half-step
$\vv{A}$ convention from commit 9769217 carries through unchanged.

The proposed leapfrog step from $n$ to $n+1$:

\begin{algsteps}
  \item \textbf{Read fields at $(\vv{x}^n, t^n)$.}
    Compute at the particle position via the existing TSC
    interpolators and the two new mixed-derivative gathers:
    $\vv{A}^n$ (the time-average $(A_{\mathrm{prev}} +
    A_{\mathrm{curr}})/2$, then spatially interpolated),
    $\partial_i \phi^n$, and $\partial_j A_i$ for all four
    $(i, j)$ pairs.
  \item \textbf{Compute $\vv{v}^{n-1/2}$ from $\vv{p}_c^{n-1/2}$
    and $\vv{A}^{n-1/2}$.}
    Read $\vv{A}^{n-1/2}$ at $\vv{x}^n$ (the time-staggered
    field at the time-symmetric position).
    Define $\vv{p}_{\mathrm{kin}}^{n-1/2} = \vv{p}_c^{n-1/2}
    - q\vv{A}^{n-1/2}$.
    Compute $\gamma^{n-1/2} = \sqrt{1 +
    |\vv{p}_{\mathrm{kin}}^{n-1/2}|^2/(m c)^2}$ and
    $\vv{v}^{n-1/2} = \vv{p}_{\mathrm{kin}}^{n-1/2}/(\gamma^{n-1/2} m)$.
  \item \textbf{Kick.}
    Compute the discrete force using Eqs.~\eqref{eq:gempic_Fx}--\eqref{eq:gempic_Fy}
    with $\vv{v} = \vv{v}^{n-1/2}$ (first-order in time on the
    velocity but second-order on the field reads since the
    field gradients are at $t^n$):
    \begin{equation}
    \vv{F}^n \;=\; q\,(\vv{v}^{n-1/2}\cdot\nabla)\vv{A}|_{n}
                - q\,\nabla\phi|_{n}.
    \end{equation}
    Wait -- this is $q (\vv{v}\cdot\nabla)\vv{A}$, NOT
    $q\nabla(\vv{v}\cdot\vv{A})$.  Which is correct?

    From Eq.~\eqref{eq:gempic_pcdot}:
    $\dot{p}_{c,i} = q v_j \partial_i A_j - q \partial_i \phi$.
    The free index $i$ is the component of $\vv{F}$; the summed
    index $j$ is the component of $\vv{v}$ that pairs with the
    component of $\vv{A}$ being differentiated.  So we need
    $\partial_i A_j$ where $i$ is the FORCE direction and $j$ is
    the velocity component.

    For 2D:
    \begin{align}
    F_x &= q\,v_x\,\partial_x A_x + q\,v_y\,\partial_x A_y - q\,\partial_x\phi,\\
    F_y &= q\,v_x\,\partial_y A_x + q\,v_y\,\partial_y A_y - q\,\partial_y\phi,
    \end{align}
    matching Eqs.~\eqref{eq:gempic_Fx}--\eqref{eq:gempic_Fy}.

    Apply the kick:
    \begin{equation}
    \label{eq:gempic_kick}
    \vv{p}_c^{n+1/2} \;=\; \vv{p}_c^{n-1/2} + \vv{F}^n\,\Delta t.
    \end{equation}
  \item \textbf{Drift.}
    Compute $\vv{v}^{n+1/2}$ from $\vv{p}_c^{n+1/2}$ and
    $\vv{A}^{n+1/2}$ via the same algebra as Step 2 (read
    $\vv{A}^{n+1/2}$ at $\vv{x}^n$, subtract, compute $\gamma$
    and $\vv{v}$).  Then
    \begin{equation}
    \label{eq:gempic_drift}
    \vv{x}^{n+1} \;=\; \vv{x}^n + \vv{v}^{n+1/2}\,\Delta t.
    \end{equation}
\end{algsteps}

This is explicit -- no fixed-point iteration is required at the
non-relativistic level because $\gamma$ is computed from
$\vv{p}_{\mathrm{kin}}$ directly, and $\vv{p}_{\mathrm{kin}}$ is
just $\vv{p}_c - q\vv{A}$.

\subsubsection*{Note on time-centering of $\vv{v}$ in the force}

The Hamilton's-equations derivation places $\vv{v}$ at the same
instant as the force, namely $t^n$.  The discrete scheme above uses
$\vv{v}^{n-1/2}$, which is half a step early.  For a leapfrog
structure, the canonical move is to use the time-symmetric average
$\vv{v}^n \approx (\vv{v}^{n-1/2} + \vv{v}^{n+1/2})/2$, but
$\vv{v}^{n+1/2}$ is not available until after the kick.

Three options:
\begin{enumerate}
  \item \textbf{Iterate} -- predictor with $\vv{v}^{n-1/2}$ to get
    a tentative $\vv{p}_c^{n+1/2}$ and hence $\vv{v}^{n+1/2}$,
    then recompute the kick with the averaged velocity.  This
    converges in one Newton-like step at non-relativistic $v$
    and a few at relativistic.  Cost: $\sim 2\times$ the bare
    scheme.
  \item \textbf{Accept the half-step lag} -- use $\vv{v}^{n-1/2}$
    in the force.  Loses second-order time accuracy on the
    $\vv{v}\cdot\nabla\vv{A}$ contribution (still
    second-order on $-\nabla\phi$).  For Stage 37's uniform-motion
    test the difference is irrelevant ($\vv{v}$ is constant).
  \item \textbf{Use a different splitting} -- DKD (drift-kick-drift)
    where $\vv{x}$ advances by $\Delta t/2$ first, then a full
    kick at the half-step position, then another $\Delta t/2$
    drift.  This is the velocity-Verlet structure and is
    symplectic for separable Hamiltonians.  Our Hamiltonian is
    not separable, but DKD with appropriate field reads should
    still give second-order accuracy and good energy behaviour.
\end{enumerate}

Default proposal: \textbf{option 2} (accept the half-step lag) as
the first GEMPIC implementation, since it is the simplest and
already gives correct uniform-motion behaviour (verified
analytically below).  Iterate later if the orbit-test radiation
reaction is qualitatively wrong.

\subsubsection*{Note on $\vv{A}$ time-staggering}

The half-step $\vv{A}$ convention (Aprev $= \vv{A}^{n-1/2}$,
Acurr $= \vv{A}^{n+1/2}$) lets us read $\vv{A}$ at three relevant
times via simple combinations:
\begin{align}
\vv{A}^{n-1/2}\;&\text{from \texttt{prev}},\\
\vv{A}^{n+1/2}\;&\text{from \texttt{curr}},\\
\vv{A}^{n}\;&= \tfrac{1}{2}(\vv{A}^{n-1/2} + \vv{A}^{n+1/2})\quad
              \text{(second-order time interpolation)}.
\end{align}
For the canonical-to-kinetic momentum conversion in Step 2 we use
$\vv{A}^{n-1/2}$; in Step 4 we use $\vv{A}^{n+1/2}$.  The force
itself in Step 3 reads field gradients at $t^n$, which means using
$\vv{A}^n$ (the average) for the $\partial_j A_i$ gathers.

\subsection{Analytic verification: zero force on uniform motion}

The structural claim is that the GEMPIC discrete force law gives
identically zero force on a uniformly-moving deposit.  Verify by
the following argument.

\subsubsection*{Setup}

A single particle moves with constant velocity $\vv{v}_0$.  Its
position at time $t$ is $\vv{x}_p(t) = \vv{x}_0 + \vv{v}_0 t$.
The fields $\phi$ and $\vv{A}$ at any time $t$ are produced by the
particle's own past trajectory via the discrete wave equations
sourced by $\rho$ and $\vv{J}$ deposited by the particle.  For
constant motion, the wake of the deposit is a steadily-translating
pattern that travels with the particle in the long-time limit
(the boosted-Coulomb / boosted-vector-potential pattern).

In this steady state, the field at the particle position is
constant in the particle's rest frame: $\phi(\vv{x}_p(t), t)$ and
$\vv{A}(\vv{x}_p(t), t)$ are time-independent.

\subsubsection*{Equation of motion in the steady state}

For uniform motion, we need
$\dot{\vv{p}}_c = 0$ and $\dot{\vv{x}} = \vv{v}_0$.
The second follows from $\vv{p}_c = \gamma m \vv{v}_0 +
q\vv{A}(\vv{x}_p(t), t)$ being constant (so $\vv{v}$ is constant).
The first requires the force in Eq.~\eqref{eq:gempic_pcdot} to
vanish:
\begin{equation}
\label{eq:gempic_zero_force_condition}
q\, v_{0,j}\,\partial_i A_j \;-\; q\,\partial_i \phi \;=\; 0
\quad\text{for } i = x, y.
\end{equation}

In the continuum, this is a property of the boosted-Coulomb /
boosted-vector-potential solution: $\phi(\vv{x}_p)$ and
$\vv{A}(\vv{x}_p)$ satisfy
$\vv{v}\cdot\vv{A}(\vv{x}_p) = \phi(\vv{x}_p) +$ (boost-invariant
constant), and at the symmetric point of the wake pattern $\nabla$
gives the same gradient on both sides modulo a sign.

The question is: does the \emph{discrete} scheme preserve
Eq.~\eqref{eq:gempic_zero_force_condition}?

\subsubsection*{The discrete cancellation}

In the discrete scheme, the deposit-to-field map is the same
operator for $\phi$ (from $\rho$) and for $\vv{A}$ (from $\vv{J}$).
Both pass through the SAME discrete wave equation (with the same
$c_{\mathrm{eff}}$, the same Yee Laplacian, the same PML), and both
are sourced by deposits from the same particle position.  The only
difference: $\rho = q \, S(\vv{x} - \vv{x}_p)$ while
$\vv{J} = q\vv{v}\, S(\vv{x} - \vv{x}_p)$, where $S$ is the deposit
shape (TSC, with Esirkepov continuity for $\vv{J}$).

Hence in the long-time steady state, the field patterns satisfy
\begin{equation}
\vv{A}(\vv{x}, t) \;=\; \vv{v}_0\, \phi(\vv{x}, t)
\end{equation}
\emph{at the level of the discrete operators}, provided the
deposit shape for $\rho$ and the deposit shape for $\vv{J}$ are
the same (they are: both TSC in this scheme), the wave equation
is the same (it is), and the PML treats $\phi$ and $\vv{A}$
symmetrically (it does).

[\textbf{Open item:} this argument is heuristic; the rigorous
discrete statement is that the discrete wave-equation Green's
function is the same scalar operator applied to $\rho$ or to each
component of $\vv{J}$.  A pencil-and-paper Fourier analysis of the
discrete Yee wave equation should verify the proportionality
$\hat{A}_j(\vv{k}, \omega) = v_{0,j}\,\hat{\phi}(\vv{k}, \omega)$
for a deposit moving with constant velocity.  This is the formal
work that should accompany the implementation.]

Given the proportionality $\vv{A} = \vv{v}_0\,\phi$, the
zero-force condition Eq.~\eqref{eq:gempic_zero_force_condition}
becomes
\begin{equation}
q\, v_{0,j}\, \partial_i (v_{0,j}\,\phi)
   - q\,\partial_i\phi
\;=\; q\,(|\vv{v}_0|^2 - 1)\,\partial_i\phi,
\end{equation}
which (in units with $c=1$) is $q\,(v^2 - 1)\,\partial_i\phi$.  At
the particle position by the Hockney-Eastwood matched-shape
property the spatial gradient $\partial_i \phi$ at the particle
is the same as the gradient of the deposit by itself, which
\emph{is non-zero} unless we use a centred-symmetric deposit/gather
pair.

This is the structural cancellation we want.  Two important
observations:

\begin{itemize}
  \item The cancellation does \emph{not} depend on
    $\partial_i\phi(\vv{x}_p) = 0$ (the v=0 Hockney-Eastwood
    condition).  It depends on
    $\vv{A} = \vv{v}_0 \phi$ as fields, which is a property of
    the deposit and wave equation.
  \item The relativistic prefactor $(v^2 - 1)/c^2 = -1/\gamma^2$
    in the relativistic case modifies the result; in the
    non-relativistic limit $v \ll c$ the factor approaches $-1$
    and the force is exactly $-q \partial_i \phi$ instead of
    zero.  This says the canonical force law on a uniformly-
    moving non-relativistic charge is just $-q\nabla\phi$ at the
    moving-deposit's $\phi$.  The HE adjoint condition then
    delivers the zero net force.
\end{itemize}

So GEMPIC's structural cancellation routes through the
\emph{discrete} property that $\rho$ and $\vv{J}$ source the same
wave equation, plus the existing matched-shape HE adjoint between
$\rho$ deposit and $\phi$-gradient gather at $v=0$.  Both of these
are already in the scheme; we are just leveraging them through a
different particle EOM.

\subsubsection*{Why the current Lorentz-form scheme fails the same test}

The current scheme uses
\begin{equation}
\vv{F}_{\mathrm{Lorentz}}
 = -q\nabla_h\phi - q\,\partial^h_t\vv{A} + q\vv{v}\times(\nabla_h\times\vv{A}).
\end{equation}
The discrete $\partial^h_t\vv{A}$ is a centred (or one-sided)
time-difference on the $\vv{A}$ buffers.  On a uniformly-moving
deposit, the steady-state $\vv{A}$ at the particle is
\emph{time-independent in the particle's frame} but
\emph{moving relative to the grid}.  The discrete
$\partial^h_t\vv{A}$ at the lab-frame particle position picks up
the convective contribution $(\vv{v}\cdot\nabla)\vv{A}$, but
discretised on a STAGGERED time stencil while
$\nabla\phi$ is on a SPATIAL stencil at a different time slice.
There is no algebraic identity at the discrete level that makes
these two discrete operations cancel to the residual order Stage
37 needs.

GEMPIC routes around this by never computing
$\partial^h_t\vv{A}$ at all.

\subsection{Addendum (2026-05-24): the analytic argument above is incomplete; empirical confirmation that GEMPIC is not a structural fix}
\label{sec:gempic_addendum}

The ``Analytic verification'' subsection above asserts the discrete
GEMPIC force on a uniformly-moving deposit vanishes by leveraging the
HE matched-shape adjoint.  That claim is INCOMPLETE.  This addendum
corrects it.

\subsubsection*{The corrected Fourier analysis}

The discrete Yee wave operator symbol is the same on every sublattice:
$$
\hat L(\vv k, \omega)
   = \frac{4}{\dd t^2}\sin^2\!\Big(\tfrac{\omega\,\dd t}{2}\Big)
   - \frac{4 c^2}{\dd x^2}\Big[\sin^2\!\Big(\tfrac{k_x\,\dd x}{2}\Big)
                                  + \sin^2\!\Big(\tfrac{k_y\,\dd x}{2}\Big)\Big].
$$
The sources for a uniformly-moving deposit (TSC $\rho$ on corners,
Esirkepov-TSC $\vv J$ on edges) satisfy
$\hat J_j(\vv k, \omega) \approx v_{0,j}\,\hat\rho(\vv k, \omega)$ to
leading order (sinc corrections appear at $O((k v_0 \dd t)^2)$ from
the path-integration over the time step).  Passing both through the
identical discrete wave operator gives the proportionality
$$
\hat A_j(\vv k, \omega) = \frac{v_{0,j}}{c^2}\,\hat\phi(\vv k, \omega)
                          \;[1 + O(k v_0 \dd t)^2].
$$
...
$$
F_i \;=\; -\,q\Big(1 - \frac{v_0^2}{c^2}\Big)\,\partial^h_i\phi\big|_{\vv x_p}
       \;=\; -\frac{q}{\gamma^2}\,\partial^h_i\phi\big|_{\vv x_p}.
$$
For zero net force this requires
$\partial^h_i\phi|_{\vv x_p} = 0$.  The original v37 argument claimed
this followed from the HE matched-shape adjoint.  \textbf{That was
wrong}: HE matched-shape only guarantees
$\partial^h_i\phi|_{\vv x_p} = 0$ at $v = 0$ (the stationary case).
For a moving deposit, $\partial^h_i\phi|_{\vv x_p}$ is precisely the
quantity Stage 37's ``phi-only'' force measures, and it is non-zero
on the grid -- in fact UV-divergent in $\dd x$ per the Stage 37 $\dd x$
convergence scan.

\subsubsection*{Empirical confirmation (Stage 41)}

GEMPIC was implemented behind a default-off flag
(\texttt{gr\_sim\_set\_pusher(sim, GR\_PUSHER\_GEMPIC)}) and
\texttt{stage41\_gempic\_check.c} ran the Stage 37 setup with both
pushers side by side, reporting per-step spurious acceleration:
...
\midrule
$0.001$ & $+1.04\times10^{-6}$ & $+1.03\times10^{-6}$ & $0.986$ \\
$0.01$  & $-4.75\times10^{-6}$ & $-4.75\times10^{-6}$ & $0.999$ \\
$0.05$  & $+4.87\times10^{-8}$ & $+4.93\times10^{-8}$ & $1.012$ \\
$0.1$   & $+2.31\times10^{-7}$ & $+2.24\times10^{-7}$ & $0.969$ \\
$0.2$   & $-2.71\times10^{-7}$ & $-2.14\times10^{-7}$ & $0.790$ \\
$0.3$   & $+4.95\times10^{-8}$ & $-2.66\times10^{-7}$ & $5.4\times$ (sign flip) \\
\bottomrule
\end{tabular}
\end{center}

At every $v$ in the sweep, the GEMPIC residual is within a few percent
of the Boris residual.  At $v = 0.3$ the discrete Boris cancellation
happens to be unusually clean, so GEMPIC is actually slightly worse
there.

\subsubsection*{Implication}

The structural problem behind Stage 37 lives in the
\emph{deposit-and-gather pair geometry}, NOT in the pusher form:

\begin{itemize}
  \item Both pushers' residuals reduce to a prefactor times
    $\partial^h_y\phi|_{\vv x_p}$ on the discrete grid.
  \item The discrete $\partial^h_y\phi|_{\vv x_p}$ for a moving
    deposit is non-zero, with the magnitude Stage 37 measures, and
    grows as $\dd x$ shrinks (UV-divergent).
  \item The HE matched-shape adjoint kills this gradient only at
    $v = 0$.  At $v \ne 0$ a stronger condition is needed (matched-
    shape adjoint at non-zero velocity), and we do not have a
    construction that delivers it on TSC + Esirkepov + Yee.
\end{itemize}

The empirical paths that DO shrink the artifact, per Stage 37's
smoothing scan, all act on the effective deposit width:
\begin{itemize}
  \item Smoothing the deposit (Stage 37: 16 binomial passes reduces
    the phi-only artifact $\sim 200\times$).
  \item A wider macroparticle (equivalent to smoothing in the
    high-$k$ content).
  \item Higher-order Yee schemes (better $k$-space isotropy, marginal
    improvement expected).
  \item Spectral / pseudo-spectral wave equations (no grid dispersion;
    much more expensive).
\end{itemize}

\textbf{GEMPIC is therefore not the principled fix Stage 37 calls for.}
The pusher is correct (no $-\partial^h_t\vv A$ to break the
cancellation), but the cancellation that needs to happen is on the
\emph{field} side at the particle position, not on the \emph{pusher}
side, and GEMPIC leaves the field-side cancellation unchanged.

\subsubsection*{Status of the implementation}

GEMPIC stays in the tree as a default-off diagnostic
(\texttt{gr\_sim\_set\_pusher} API, \texttt{gempic\_em\_kick} in
\texttt{core/src/particle.c}, two new LB gathers in
\texttt{core/src/deposit.c}, \texttt{stage41\_gempic\_check.c}).
The infrastructure is reusable if a future scheme combines a
canonical-momentum push with a different deposit-side construction.
Commit \texttt{3e0b209} carries all of this.

The ``Implementation order'' section below (and the v36
``Implementation order'' it refines) is superseded by the new
direction: \textbf{focus on the deposit-side construction, not the
pusher.}  The next scope item should be a deposit-and-gather pair
that satisfies the matched-shape adjoint condition at $v \ne 0$ --
or, failing a structural construction, a deposit-side mitigation
(fixed-physical-width macroparticle) with clear accuracy bounds
documented.

\subsection{Open items before implementation}

\begin{enumerate}
  \item \textbf{Rigorous discrete proof of
    $\hat{A}_j = v_{0,j}\,\hat{\phi}$ for moving deposit.}
    The continuum statement is trivially true (the wave equations
    are identical scalar operators on $\rho$ and each $J_j$, with
    $J_j = \rho v_j$).  The discrete statement requires that the
    Yee Laplacian on edge-stored $\vv{A}$ commutes with the
    Esirkepov current decomposition in the right sense.  Pencil-
    and-paper Fourier analysis.
  \item \textbf{Implementation of the two new mixed-derivative LB
    gathers.}  \texttt{gr\_tsc\_lb\_dy\_xedge} and
    \texttt{gr\_tsc\_lb\_dx\_yedge} in
    \texttt{core/src/deposit.c}.  Mirror the existing
    \texttt{gr\_tsc\_lb\_dx\_xedge} / \texttt{\_dy\_yedge} with the
    sublattice indexing swapped.
  \item \textbf{Particle state}: add $\vv{p}_c$ alongside
    $\vv{p}_{\mathrm{kin}}$ in \texttt{gr\_particle\_t}, or replace.
    Recommendation per v36: keep $\vv{p}_{\mathrm{kin}}$ as the
    "output" field (used for diagnostics, KE computation, etc.) and
    compute it on demand from $\vv{p}_c$ via Eq.~\eqref{eq:gempic_pc_pkin}.
    The stored canonical field is the new addition.
  \item \textbf{Feature-flag plumbing}: add
    \texttt{gr\_pusher\_kind\_t \{GR\_PUSHER\_BORIS, GR\_PUSHER\_GEMPIC\}}
    and a setter on \texttt{gr\_sim\_t}.  Default Boris.  When GEMPIC
    is selected, the particle push branches in
    \texttt{core/src/particle.c}.
  \item \textbf{Gravity-side parallel.}  The gravity sector has an
    analogous canonical structure (with $q \to m$, $\vv{A} \to
    \vv{A}_g$) and presumably the same structural defect under
    Stage 28's heating diagnostic.  Plan: do EM GEMPIC first as a
    proof of concept; replicate to gravity afterwards.  The
    benchmark suite (Stages 37--40) should be extended with
    gravity-analog cases at that time.
  \item \textbf{Time-step / dispersion considerations.}
    The non-Boris pusher does not have Boris's exact-rotation
    structure for the magnetic force.  At high $v\times B$
    (strong $\vv{B}$, large gyroradius), the leapfrog GEMPIC may
    have larger gyrofrequency dispersion than Boris.  For our
    pedagogical regime ($\gamma$ typically below 1.1) this is
    likely a non-issue, but should be measured.
\end{enumerate}

\subsection{Implementation order (refining v36's order)}

Per v36 §\ref{sec:gempic_benchmark}, the benchmark suite (B-1
through B-6) is in hand as of commits \texttt{67e22d7} (Stage 37
baseline-ready) and \texttt{0a7c2b7} (Stages 38--40 and
\texttt{make benchmark-pic}).  The next steps:

\begin{enumerate}
  \item Pencil-and-paper Fourier verification of
    $\hat{A}_j = v_{0,j}\,\hat{\phi}$ on the discrete Yee scheme.
    Out-of-band of this document; should be a one-page note.
  \item Implement the two new LB gathers
    (\texttt{gr\_tsc\_lb\_dy\_xedge},
    \texttt{gr\_tsc\_lb\_dx\_yedge}).
    Add unit tests that read a known polynomial $\vv{A}$ field and
    verify the gather equals the analytic derivative.
  \item Implement the GEMPIC pusher behind a default-off flag
    \texttt{gr\_sim\_set\_pusher(sim, GR\_PUSHER\_GEMPIC)}.
    Keep Boris as default and as legacy regression path.
  \item Run \texttt{make benchmark-pic} with GEMPIC enabled.
    Capture the table.  Compare to baseline.
  \item If the benchmark passes acceptance (per v36 sec
    gempic\_benchmark): flip the default, mark Boris as legacy.
    Update memory files.
  \item If the benchmark FAILS acceptance:
    do NOT flip the default.  Diagnose specifically which
    sub-test fails and which open item from this section was
    the cause.  Iterate.
\end{enumerate}
...
%====================================================================
\section*{Changes from v35 (this revision)}
%====================================================================

This revision documents the Stage 37 diagnostic findings, scopes the
benchmark suite that any EM PIC scheme change must pass, and scopes
the canonical-momentum (GEMPIC) reformulation that is the principled
fix for the structural defect Stage 37 exposed.

\begin{enumerate}
  \item \textbf{Stage 37 (§\ref{sec:stage37_diagnosis})}: a constant-
    velocity charged particle in vacuum should conserve energy and
    momentum exactly (continuum Maxwell self-force on uniformly moving
    charge is zero).  The current scheme fails this catastrophically:
    at $v/c=0.001$ the spurious self-force captures the particle at a
    stable fixed point $v_y \approx -0.28c$ within $\sim 50$ steps.
    The artifact is dt-independent, UV-divergent in $\Delta x$ (grows
    by $1.5\times$ on phi-only per $\Delta x$ halving), and shrinks
    only when the deposit is smoothed (effectively widening the
    macroparticle).  This is a structural failure of the discrete
    $-\nabla\phi - \partial_t\vv{A}$ cancellation, not a tunable bug.
  \item \textbf{Prior ``fixes'' re-labeled}: the half-step $\vv{A}$
    potentials change (commit \texttt{9769217}, 2026-05-21) and the
    Stage~36 inductive sign flip diagnostic (2026-05-22) both reduced
    \emph{orbit-test} drift.  Stage 37 shows neither addressed the
    underlying cancellation failure.  The half-step $\vv{A}$
    reinterpretation is still a real improvement and is kept; the
    inductive sign API is now labeled diagnostic-only.
  \item \textbf{Benchmark suite scope (§\ref{sec:gempic_benchmark})}:
    six tests with explicit acceptance criteria, designed to FAIL the
    current scheme along the dimensions where it is broken.  The
    primary acceptance gate is uniform-motion conservation of $E$ and
    $p$ to integrator-noise level; radiation-rate quantitative tests
    are deferred until a 2D Liénard--Wiechert analytic is built (2D
    retarded fields decay as $1/r$, not $1/r^2$).
  \item \textbf{GEMPIC canonical-momentum reformulation scope
    (§\ref{sec:gempic_design})}: track $\vv{p}_c = \gamma m \vv{v} + q\vv{A}$
    instead of $\vv{p}_{\mathrm{kin}}$; force law becomes
    $\dd \vv{p}_c / \dd t = q\nabla(\vv{v}\cdot\vv{A}) - q\nabla\phi$
    with no $\partial_t \vv{A}$ and no $\vv{v}\times\vv{B}$ explicitly
    (both emerge in the $\vv{p}_c \to \vv{p}_{\mathrm{kin}}$
    conversion).  The structurally-broken operator is eliminated.
  \item \textbf{Test catalog blind spot identified}: pre-Stage-37 PIC
    tests were all either orbit-based (averages over direction) or
    $v=0$ (Hockney--Eastwood).  No test of ``moving charged particle,
    measure its KE drift'' existed.  Stage 37 is now mandatory
    regression for every future EM PIC change.
\end{enumerate}

All v35 content (implementation alignment, Yee + Esirkepov migration,
the full v32 physics framework) carries forward unchanged.  Cross-
references to \texttt{eq:} and \texttt{sec:} labels remain valid.

\newpage

%====================================================================
\section{Stage 37: Uniform-Motion Self-Force Diagnosis}
\label{sec:stage37_diagnosis}
%====================================================================

\subsection{The problem statement}

A free charged particle moving at constant velocity in vacuum does
\emph{not} radiate (Larmor radiation requires acceleration).
Equivalently, in continuous Maxwell electrodynamics the self-force on
a uniformly-moving charge is identically zero: the $-\nabla\phi$ piece
(longitudinal field of the boosted Coulomb potential) is \emph{exactly}
cancelled by the $-\partial_t\vv{A}$ piece (from the time-changing
$\vv{A}$ in the boosted frame).  In a faithful PIC discretisation the
kinetic energy of such a particle must be conserved up to grid-
aliasing noise --- there is no physically-allowed channel for KE to
leak into the field.

Stage~37 (\texttt{core/tests/stage37\_em\_constant\_velocity.c}) is
the test that exercises this condition: a $128 \times 1024$ asymmetric
domain, a single $+Q$ charge launched at low $y$ with constant $+y$
velocity, no background, PML absorbers on both transverse edges.

\subsection{Convergence triangulation}

Four orthogonal refinement scans were run.  All at $v=0.30c$ with the
default production knobs (\texttt{rho\_smooth=4}, TSC$+$LB), except
the smoothing scan which scans the smoothing.  Results are KE drift
percentages over $\sim 4000$ steps.

\subsubsection*{Smoothing scan (passes applied to BOTH $\rho$ and $\vv{J}$)}
...
Smoothing widens the macroparticle and damps the high-$k$ content of
the deposit; the artifact follows it.  The original ``baseline''
phi-only of $+16\%$ was actually 4-pass smoothed; the bare phi-only
self-force in this scheme is $+1587\%$.

\subsubsection*{CFL / dt scan (\texttt{dx}, \texttt{c\_eff}, $v$ all held fixed; only $\dd t$ varies)}
...
$\dd t$ cut by $5.66\times$ produced under $10\%$ change in drift.
Not a time-truncation error.  Refining $\dd t$ does \emph{not} help.

\subsubsection*{$\Delta x$ convergence scan (physical setup held fixed; $W$, $H$, $n_{\mathrm{damp}}$, $\dd t$ all scale with $\Delta x$)}

\begin{center}
\begin{tabular}{rrrrl}
\toprule
$\Delta x$ & n\_steps & phi-only  & full       & halving ratio \\
\midrule
...
Halving $\Delta x$ \emph{grows} the phi-only artifact by $53\%$.
Ratio $> 1$ means UV-divergent: the artifact is the discrete shadow
of an IR-divergent point-source self-energy that the implicit
$\sim \Delta x$ deposit regularisation was hiding.  As $\Delta x$
shrinks the regularisation shrinks with it, exposing more of the
divergence.

\subsubsection*{$v_{\mathrm{drift}}$ scan (with low-$y$ bail-out)}

\begin{center}
\begin{tabular}{rrrrr}
\toprule
$v/c$  & $\Delta v_y$ (phi) & $\Delta v_y$ (full) & cells & KE\% (phi)    \\
\midrule
...
At $v=0.001$ AND $v=0.01$ the spurious force \emph{reverses} the
particle's velocity within $\sim 50$ steps and drives it to a stable
equilibrium near $v_y \approx -0.28c$.  The dvy values at low $v$ are
not ``force at $v=v_{\mathrm{init}}$'' --- they are ``force has a
stable fixed point at $v \approx -0.28c$ that captures any low-$v$
particle.''  The artifact is \emph{worst} at non-relativistic $v$,
not mildest.

\subsection{Combined picture}
...
\midrule
Refine $\dd t$            & CFL down $5.7\times$         & unchanged        \\
Refine $\Delta x$         & halve                    & grows $\sim 50\%$ (phi) \\
Smooth deposit     & passes 0 $\to$ 16          & shrinks $200\times$    \\
Shrink $v$ (c\_eff $\uparrow$)  & prior c\_eff scan & shrinks to 0 \\
Shrink $v_{\mathrm{drift}}$  & v scan, this section & gets MUCH worse  \\
\bottomrule
\end{tabular}
\end{center}

This is a textbook UV-sensitive structural failure of the discrete
$-\nabla\phi + (-\partial_t\vv{A})$ cancellation on a moving deposit.
The two operators do not constitute a discrete adjoint pair at finite
sub-cell motion; the residual is the self-force of the regularised
delta function moving across the grid.  Refining $\Delta x$ tightens
the implicit regularisation, the effective point-source gets sharper,
and the spurious self-force \emph{grows}.

\subsection{What this implies for prior ``fixes''}

\textbf{Half-step $\vv{A}$ potentials (commit 9769217, 2026-05-21).}
The reinterpretation of $\vv{A}_{\mathrm{prev}} = \vv{A}^{n-1/2}$,
$\vv{A}_{\mathrm{curr}} = \vv{A}^{n+1/2}$ giving $\partial_t \vv{A}^n =
(\vv{A}_{\mathrm{curr}} - \vv{A}_{\mathrm{prev}})/\dd t$ on a 1-step
stencil naturally aligned with $\vv{J}^{n-1/2}$ was a real
improvement.  It reduced the Stage 27 EM PIC orbit drift from
$+18.6\%$ outspiral to $-9.2\%$ inspiral, and corresponding numbers in
gravity.  At the orbit-test level, the inductive heating was gone.
Stage 37 shows the \emph{operator pair} still fails to cancel on a
moving deposit; the orbit-test improvement masked but did not remove
the structural defect.  The half-step change is kept (better
time-staggering, 1-step stencil); it is now correctly labeled as
\emph{partial mitigation}, not a structural fix.

\textbf{Stage 36 inductive sign flip (2026-05-22).}  Setting
\texttt{em\_inductive\_sign = -1} cosmetically rotates Stage 27/30
orbit drift from $+25\%$ outspiral to $-3.5\%$ inspiral.  The
quantitative evidence from Stage 37 (uniform motion) shows that
inverting the sign of the inductive contribution would change the
$-78\%$ inductive-only drift to $+78\%$ --- equally broken, opposite
sign.  No choice of sign on a broken operator pair fixes a
cancellation that does not exist.  The API knob
\texttt{gr\_sim\_set\_em\_inductive\_sign} remains as a diagnostic
probe with its docstring updated to label it diagnostic-only and warn
against using it to ``fix'' orbit drift.  Default sign stays $+1$ (the
variationally-derived value).

\subsection{Test catalog blind spot}

None of the existing $\sim 36$ stage tests exercised the failure mode
Stage 37 exposes.  The blind spot is coherent and worth recording so
future test design avoids it:

\begin{itemize}
  \item \textbf{Field-only tests (Stages 1--6, 13--17)}: no particles
    deposit anything.  Cannot probe self-force.
  \item \textbf{Particle-in-applied-field tests (Stages 7--9,
    20--25)}: a particle feels a known background field without
    depositing.  Tests the force law given a field, not the
    deposit/field/force loop.
  \item \textbf{Stage 10 ``perturbation FDTD with moving particle
    source''}: closest existing test.  Tests A and C use a stationary
    particle.  Test B uses a moving particle but measures only the
    boosted field shape ($|\vv{A}_g|/|\Phi_g| \approx v/c^2$) at a
    distance, not the force back on the source.  Test E measures
    orbital period accuracy.  No test of ``moving charged particle,
    measure its KE drift'' was present.  The
    \texttt{pic\_constant\_v} scenario used by Test B sets
    \texttt{charge = 0.0f} (a neutral particle) and therefore cannot
    test EM self-force at all.
  \item \textbf{Stage 11 Esirkepov continuity}: verifies
    $\partial\rho/\partial t + \nabla\cdot\vv{J} = 0$.  Necessary
    condition for gauge consistency but does NOT imply $-\nabla\phi$
    and $-\partial_t\vv{A}$ cancel on a moving deposit.  Different
    invariant; we were checking the wrong thing for self-force.
  \item \textbf{Orbit tests (Stages 12, 18, 18b, 27, 30, 32)}: the
    velocity vector rotates through all $360^\circ$ per period.  A
    directional spurious force partially averages out around a loop.
    The orbital (central) force is $\sim 10^2 \times$ the spurious
    force, so the artifact appears as a small percentage-per-orbit
    instead of a flip-the-velocity-in-50-steps as it does on the
    uniform-motion test.  The Stage 27/30 $+25\%$ outspiral WAS this
    artifact, but was interpreted as a sign-convention puzzle.
  \item \textbf{Hockney--Eastwood static self-force tests (Stage 10C,
    Stage 33)}: verify zero self-force at $v=0$.  HE only guarantees
    zero self-force at zero velocity; nobody tested $v \ne 0$ until
    Stage 37.
\end{itemize}

Implication for the suite design in §\ref{sec:gempic_benchmark}:
tests must probe the deposit$\to$field$\to$force-on-moving-source
chain as an independent invariant, not rely on orbit-level cumulative
drift, not rely on $v=0$ HE, not rely on charge conservation as a
proxy for force balance.

\newpage

%====================================================================
\section{Benchmark Suite for EM PIC Scheme Changes}
\label{sec:gempic_benchmark}
%====================================================================

This section formalises the regression and acceptance suite that any
EM PIC scheme change --- the planned GEMPIC canonical-momentum
reformulation (§\ref{sec:gempic_design}) or any alternative --- must
pass before being declared a fix.  The suite is designed to FAIL the
current scheme along the dimensions where it is genuinely broken, so
an acceptance run gives clean signal about whether a candidate fix
actually addresses the structural problem (vs.\ rotating the
symptom).

\subsection{Design principles}

\begin{enumerate}
  \item Tests are designed to BREAK candidates, not to confirm them.
    Each test states the failure modes it is built to expose.
  \item Acceptance criteria are quantitative absolute numbers, not
    pass/fail strings or ``drift looks reasonable'' judgements.
  \item Tests report results in BOTH percent and absolute units (KE
    in energy units, momentum in $mc$, force in $q^2/\Delta x^2$).
    At low $v$ the percent metric is uninformative; absolute is the
    gate.
  \item Every test runs in two sweep configurations: current scheme
    (baseline, expected fail) and candidate fix.  Both sets of
    numbers go in the report.  A fix that breaks an existing-passing
    regression is not acceptable.
  \item No acceptance number depends on a quantity that any of the
    diagnostic knobs (\texttt{em\_inductive\_sign},
    \texttt{em\_inductive\_disc}, \texttt{j\_deposit\_shift},
    \texttt{em\_magnetic\_enabled}) can rotate.  These knobs are
    diagnostic-only.
  \item Runner defaults are $\rho_{\mathrm{smooth}}=0$,
    $j_{\mathrm{smooth}}=0$.  The bare deposit is the honest baseline.
    Smoothed runs are explicit add-on configs, never the default.
\end{enumerate}

\subsection{Primary acceptance gate (necessary condition)}

A uniformly-moving free particle MUST conserve its energy and momentum
to integrator-noise level, at every $v$ in the test sweep, with the
bare unsmoothed deposit ($\rho_{\mathrm{smooth}}=0$,
$j_{\mathrm{smooth}}=0$).

This is the \emph{necessary} condition.  Anything that fails it is not
a candidate fix, regardless of how well it does on orbit tests.
Tests \textbf{B-1} (single particle) and \textbf{B-2} (neutral pair)
gate this condition.

\subsection{Secondary acceptance (sufficient for production trust)}

Static reciprocity (\textbf{B-3}), basic two-body dynamics
(\textbf{B-4}), and the existing PIC regressions (\textbf{B-5})
must also hold.  Together with the primary gate, this is the
sufficient set for declaring the scheme production-trusted.

\subsection{Exploratory / report-only (NOT acceptance gates)}

Radiation-reaction quantitative tests (\textbf{B-6}) are exploratory.
Quantitative agreement with a 2D Larmor formula is NOT a hard gate
because:

\begin{itemize}
  \item In 2D the retarded fields decay as $1/r$, not $1/r^2$ as in
    3D.  Total radiated power has a different scaling and prefactor
    from the textbook 3D Larmor formula.
  \item The correct analytic benchmark is the 2D Liénard--Wiechert
    potentials.  These are not currently implemented in GRlite, and
    deriving them is itself a substantial separate task.
  \item A scheme that passes the necessary conditions above might
    still give the ``wrong'' absolute radiation rate in 2D for
    reasons unrelated to the uniform-motion artifact (grid
    dispersion, PML residual, etc.).
\end{itemize}

B-6 is run AFTER B-1--B-5 are passing, and produces a qualitative
report (``orbit inspirals monotonically; loss rate is non-zero and
scales with $v^k$ for some empirical $k$'') rather than a hard
threshold.  A separate scope item, deferred, is to derive the 2D
Liénard--Wiechert potential, add a 2D Larmor analytic, and then
promote B-6 to a real acceptance gate.

\subsection{Tests}

\subsubsection*{B-1.  Stage 37 uniform motion, expanded}

\emph{Status: partially exists; needs the following extensions before
the baseline run.}

\begin{itemize}
  \item Strip the $\rho_{\mathrm{smooth}}=4$ default from the runner;
    run at $\rho_{\mathrm{smooth}}=0$ for the primary scan.
  \item v-sweep at $v/c \in \{0.001, 0.01, 0.05, 0.1, 0.2, 0.3, 0.5,
    0.7\}$.
  \item Add per-step force measurement, not just integrated KE drift:
    average $\Delta v_y / \Delta t$ over a short window after warm-up,
    reported in absolute units ($\dd v / \dd t \cdot \tau$, where
    $\tau$ is a fixed physical reference timescale).  This factors
    out the ``particle captured at $v_y \approx -0.28c$'' pathology.
  \item Report $\Delta x$ convergence and CFL invariance on the
    v-sweep, not just $v = 0.3$.
\end{itemize}

Acceptance for a fix:
\begin{itemize}
  \item $|\Delta v_y / \Delta t| <$ integrator-noise floor (measured
    from the no-force config in the same Stage 37) at every $v$ in
    the sweep, with $\rho_{\mathrm{smooth}}=0$.  Absolute threshold
    to be set from the baseline run.
  \item $|KE_{\mathrm{final}} - KE_{\mathrm{initial}}| / KE_{\mathrm{initial}} < 1\%$ at $v \ge 0.1$
    and $|\Delta v_y / v_{y,\mathrm{init}}| < 1\%$ at $v < 0.1$.
  \item $\Delta x$ halving ratio $\in (0.25, 1.0]$ --- convergent or
    constant, never growing.
  \item No sign-of-force flip across the v sweep.  A working scheme
    has no net longitudinal force at any $v$.
\end{itemize}

\subsubsection*{B-2.  Neutral pair co-moving (NEW, Stage 38)}

\emph{Setup:} two equal-and-opposite charges ($+Q$, $-Q$), both at
velocity $(0, +v)$ along the same $y$-axis line, separated by a
transverse distance $\Delta x_{\mathrm{sep}}$ in the $x$-direction.
Net $\rho$ is zero modulo the finite extent of the deposit shape; net
$\vv{J}$ is zero ($\vv{J}_{+Q} + \vv{J}_{-Q} = 0$).  At long range
the joint field is dipole, not monopole.

\emph{Physics expectation:} each particle's KE is conserved up to
dipole-wake radiation (very small at modest $v$).  NO spurious
self-force: the joint field is structurally smaller than either
particle's individual wake.

\emph{Why this probe:} isolates self-force-on-moving-deposit from
``interaction with own wake''.  The current scheme's UV-divergent
self-force will still hit each particle independently, and both will
drift to the same fixed point at $v_y \approx -0.28c$ since the fixed
point does not depend on field source.

Acceptance for a fix:
\begin{itemize}
  \item $|\Delta v_y| < 10^{-6}$ for both particles over 5000 steps
    at $v_{\mathrm{init}} = 0.001$.
  \item $|\Delta v_y| < 10^{-4}$ for both particles over 5000 steps
    at $v_{\mathrm{init}} = 0.1$.
  \item Total momentum $\vv{P}_{+Q} + \vv{P}_{-Q}$ conserved to
    machine precision modulo PML absorption.
\end{itemize}

\subsubsection*{B-3.  Reciprocal force action-reaction (NEW, Stage 39)}

\emph{Setup:} two like charges ($+Q$, $+Q$), one at rest at
$(c_x, c_y)$, one at rest at $(c_x, c_y + r)$.  Static configuration.
Measure the force on each particle via the existing
force-evaluation hooks.

\emph{Physics expectation:} $\vv{F}_1 + \vv{F}_2 = 0$ to numerical
noise (continuous Newton's third law).  Each particle sees the other's
static Coulomb field plus its own self-force ($= 0$ by Hockney--
Eastwood).

Acceptance for a fix:
\begin{itemize}
  \item $|\vv{F}_1 + \vv{F}_2| / |\vv{F}_1| < 10^{-6}$ at all
    separations $r \ge 4$ cells.
  \item This already holds for the current scheme at $v=0$ (Hockney--
    Eastwood).  The point is to ensure GEMPIC's modified force law
    does not accidentally break static reciprocity.
\end{itemize}

\subsubsection*{B-4.  Co-moving like charges (NEW, addition to Stage 38 or new file)}

\emph{Setup:} two like charges at the same $v$ in the $y$-direction,
separated transverse-wise.  Identical to B-2 except both are $+Q$.

\emph{Physics expectation:} mutual Coulomb repulsion (perpendicular
to motion) plus self-force on each.  With perfect self-force
cancellation, only mutual repulsion remains, and the test reduces to
``uniform-motion two-particle, predictable lateral expansion''
matching boosted Coulomb.

\emph{Why this probe:} distinguishes ``self-force still broken'' from
``mutual force still broken''.  If lateral expansion matches the
analytic prediction but particles are dragged backward in $y$,
self-force is broken and mutual is fine.  The current scheme will
fail in both channels.

Acceptance for a fix:
\begin{itemize}
  \item Lateral expansion within $5\%$ of the boosted-Coulomb
    prediction at $v=0.1$ over 5000 steps.
  \item $y$-velocity drift of each particle $< 1\%$ over the run.
\end{itemize}

\subsubsection*{B-5.  Existing PIC regressions}

The following must NOT get worse than current numbers under any
candidate fix.  The baseline run records current values.

\begin{itemize}
  \item Stage 11 (Esirkepov continuity): $\nabla\cdot\vv{J} +
    \partial\rho/\partial t = 0$ to machine precision.  GEMPIC must
    preserve this.
  \item Stage 18 / 18b (Lewis--Birdsall multi-orbit, gravity):
    GEMPIC for EM should not perturb gravity.
  \item Stage 27 (EM PIC orbit): current value $+25\%$ outspiral at
    sign$=+1$ with $\rho_{\mathrm{smooth}}=4$.  Acceptance for a fix:
    physically-correct inspiral (back-reaction direction; magnitude
    not gated due to the 2D Larmor caveat), with no need for
    sign-flip or special smoothing.
  \item Stage 28 (gravity inductive heating): re-run with the
    gravity-is-attractive caveat (the original ``gravity doesn't
    heat'' conclusion was downstream of the always-attractive sign
    of gravity).  Acceptance: monotone inspiral; no sensitivity to
    coupling strength.
  \item Stage 32 (EM binary PIC): bound for hundreds of orbits with
    monotone (small) inspiral.
  \item Stage 34 (EM chain signs): bit-clean.
  \item Stage 35 (EM magnetic-only): parallel-current attraction
    bit-clean.
\end{itemize}

\subsubsection*{B-6.  Radiation reaction sanity check (NEW, Stage 40 -- EXPLORATORY)}

NOT an acceptance gate.  See the ``Exploratory / report-only''
discussion above for the 2D physics reasoning.

\emph{Setup:} orbiting charge at known $r$ and $v$.  Measure energy
and angular-momentum loss rates and the orbit trajectory.

Report contents (no thresholds):
\begin{itemize}
  \item Energy loss rate vs orbit number.
  \item Angular momentum loss rate vs orbit number.
  \item Empirical $v^k$ scaling exponent over $v \in [0.05, 0.3]$.
  \item Pre-fix vs post-fix loss-rate comparison.
\end{itemize}

Follow-up (separate scope, deferred): derive 2D Liénard--Wiechert
potential, build 2D Larmor analytic, promote B-6 to acceptance gate.

\subsection{Pre-GEMPIC baseline run (mandatory)}

Before any GEMPIC implementation begins, the suite B-1 through B-6
runs on the current scheme.  Two reasons:

\begin{enumerate}
  \item Some acceptance thresholds are quoted absolute (the
    integrator-noise floor); others are relative (``better than
    current'').  The relative ones need the baseline number on
    file.
  \item Currently-passing tests may be marginal.  If Stage 11
    continuity holds to $10^{-12}$ now and GEMPIC drops it to
    $10^{-9}$, that is worse but possibly acceptable.  Without the
    baseline number we cannot judge.
\end{enumerate}

\subsection{Implementation order}

\begin{enumerate}
  \item Strip $\rho_{\mathrm{smooth}}$ default from Stage 37
    runner, add v-sweep and per-step force.  Run baseline numbers.
  \item Write Stage 38 (neutral pair).  Run baseline.
  \item Write Stage 39 (reciprocal force).  Run baseline.
  \item Write Stage 40 (radiation reaction, exploratory).  Run baseline.
  \item Bundle B-1--B-6 into a \texttt{make benchmark-pic} target
    that prints the table.
  \item ONLY THEN begin GEMPIC implementation.
\end{enumerate}

\newpage

%====================================================================
\section{GEMPIC: Canonical-Momentum Reformulation Scope}
\label{sec:gempic_design}
%====================================================================

\subsection{Reference and provenance}

The reformulation is the canonical-momentum (geometric / variational)
PIC approach following Kraus, Kormann, Morrison and Sonnendrücker
(2017), often abbreviated GEMPIC.  The relevant idea for our purposes
is not the full symplectic structure of the field-particle phase
space but the specific consequence: the discrete force law on the
particle becomes a gradient of a scalar at the particle position
($-\nabla\phi + q\nabla(\vv{v}\cdot\vv{A})$), with no
$-\partial_t\vv{A}$ and no $\vv{v}\times\vv{B}$ explicitly --- both
emerge in the conversion between canonical and kinetic momentum.

This eliminates the broken operator pair Stage 37 diagnosed: the
$-\partial_t\vv{A}$ operator that fails to be the discrete adjoint of
$-\nabla\phi$ on a moving deposit is structurally absent from the
force law.  The cancellation is no longer a property the discretisation
must preserve; it is a property the formulation does not require.

\subsection{Variables tracked on the particle}

Current scheme tracks $\vv{p}_{\mathrm{kin}} = \gamma m \vv{v}$.  GEMPIC
tracks
\begin{eqbox}
\begin{equation}
\vv{p}_c = \gamma m \vv{v} + q \vv{A}(\vv{x}_p, t).
\label{eq:gempic_pc}
\end{equation}
\end{eqbox}
This is the canonical (conjugate) momentum: the variable for which
Hamilton's equations apply cleanly when the field is present.

\subsection{The push step}

The Hamilton equations of motion for the particle in the canonical
variables are
\begin{eqbox}
\begin{align}
\frac{\dd \vv{x}_p}{\dd t} &= \vv{v} = \frac{\vv{p}_c - q\vv{A}(\vv{x}_p, t)}{\gamma m}, \label{eq:gempic_dxdt}\\
\frac{\dd \vv{p}_c}{\dd t} &= -q \nabla\phi(\vv{x}_p, t) + q \nabla\big(\vv{v}\cdot\vv{A}(\vv{x}_p, t)\big). \label{eq:gempic_dpcdt}
\end{align}
\end{eqbox}
The right-hand side of Eq.~(\ref{eq:gempic_dpcdt}) contains no
$\partial_t \vv{A}$ and no explicit $\vv{v}\times\vv{B}$.  The vector
identity
\begin{equation}
\vv{v}\times(\nabla\times\vv{A}) = \nabla(\vv{v}\cdot\vv{A}) - (\vv{v}\cdot\nabla)\vv{A}
\label{eq:vector_identity}
\end{equation}
and the chain-rule expansion of
$\dd \vv{p}_c / \dd t = \dd (\gamma m \vv{v}) / \dd t + q [\partial_t \vv{A} + (\vv{v}\cdot\nabla)\vv{A}]$
together imply that the kinetic momentum obeys the standard Lorentz
force law,
\begin{equation}
\frac{\dd (\gamma m \vv{v})}{\dd t}
= q\big(-\nabla\phi - \partial_t\vv{A} + \vv{v}\times(\nabla\times\vv{A})\big),
\end{equation}
which is the continuum equation we want.  GEMPIC differs by
\emph{which discrete operators} produce that continuum result: only
$\nabla\phi$ and $\nabla(\vv{v}\cdot\vv{A})$ are discretised on the
particle's force side; $\partial_t\vv{A}$ and
$\vv{v}\times(\nabla\times\vv{A})$ are implicit, emerging via
Eq.~(\ref{eq:gempic_pc}) when $\vv{p}_c \to \vv{p}_{\mathrm{kin}}$.

\subsection{Discretisation choices}

\subsubsection*{Time-stepping}

Eqs.~(\ref{eq:gempic_dxdt})--(\ref{eq:gempic_dpcdt}) are a coupled ODE
in $(\vv{x}_p, \vv{p}_c)$.  At leading order, a leapfrog-style
scheme is:

\begin{algsteps}
  \item Read $\vv{A}^n$ at the particle position via shape-function
    interpolation.  Compute
    $\vv{v}^{n-1/2} = (\vv{p}_c^{n-1/2} - q \vv{A}(\vv{x}_p^n, t^n)) / (\gamma m)$.
    (Note: this is implicit in $\gamma$; solve via fixed-point or
    via the standard relativistic algebra.)
  \item Advance position: $\vv{x}_p^{n+1} = \vv{x}_p^n + \vv{v}^{n-1/2} \dd t$.
    \emph{Open question: is this the right half-step indexing?  Likely
    we want $\vv{v}^n$, which means a single half-step kick before
    the drift.  Details TBD by literature check.}
  \item Read $\nabla\phi^{n}$ and $\nabla(\vv{v}^n \cdot \vv{A}^n)$ at
    the particle position via LB-style gradient gather (the
    $\dd W_3 / \dd x$ kernels already implemented in
    \texttt{core/src/deposit.c}).
  \item Advance canonical momentum:
    $\vv{p}_c^{n+1/2} = \vv{p}_c^{n-1/2} + [-q\nabla\phi^n + q\nabla(\vv{v}^n \cdot \vv{A}^n)] \dd t$.
  \item Compute $\vv{v}^{n+1/2}$ for diagnostics from
    $\vv{v}^{n+1/2} = (\vv{p}_c^{n+1/2} - q \vv{A}(\vv{x}_p^{n+1}, t^{n+1/2})) / (\gamma m)$.
\end{algsteps}

\textbf{Open issue:} the precise time-staggering of the
$\vv{A}$ reads (integer step vs half-step), the $\vv{v}$ implicit
solve, and the consistency with the existing half-step $\vv{A}$
buffer convention all need a literature pass plus a unit derivation.
Do not lock these details until the GEMPIC implementation scope is
written out in its own follow-up document.

\subsubsection*{Field-side stays the same}

The wave-equation update for $\phi$ and $\vv{A}$ is unchanged.  The
$\rho$ and $\vv{J}$ deposition stays unchanged (Esirkepov +
TSC$+$LB).  Esirkepov continuity is still required for gauge
consistency.  The only structural change is the particle push.

\subsubsection*{What about $\vv{v}\cdot\vv{A}$?}

This is a scalar at the particle position, equal to $v_x A_x + v_y A_y$
in 2D.  Its gradient $\nabla(\vv{v}\cdot\vv{A}) = v_x \nabla A_x + v_y \nabla A_y$
(treating $\vv{v}$ as held fixed during the gradient since the
gradient is at the particle position, not in $\vv{v}$-space) is two
scalar-gradient gathers on the edge-staggered $\vv{A}$ components.
The existing TSC LB infrastructure
(\texttt{gr\_tsc\_lb\_dx\_xedge}, \texttt{gr\_tsc\_lb\_dy\_yedge},
added in the Stage 37 session) provides these gathers directly.

\subsection{What GEMPIC does NOT change}

\begin{itemize}
  \item Field representation (storage layout, Yee staggering, sublattices).
  \item Wave equation update (leapfrog $\phi$, $\vv{A}$).
  \item Deposition: $\rho$ from particle to CORNER, $\vv{J}$ from
    particle via Esirkepov to X\_EDGE / Y\_EDGE.
  \item Esirkepov continuity (still required).
  \item Half-step $\vv{A}$ convention (still useful for the
    diagnostic $\partial_t \vv{A}$ if anyone ever wants to compare).
  \item Shape function and force interpolation infrastructure
    (TSC$+$LB on the deposit side, LB gradient gather on the force
    side).
\end{itemize}

\subsection{What GEMPIC DOES change}

\begin{itemize}
  \item Particle state vector: $\vv{p}_c$ replaces (or shadows)
    $\vv{p}_{\mathrm{kin}}$.  Storage: one new field on
    \texttt{gr\_particle\_t}, or a flag toggling interpretation.
  \item Particle push: new path
    \texttt{gr\_particle\_push\_gempic} alongside the existing
    Boris path.  Selected via
    \texttt{gr\_sim\_set\_particle\_pusher(sim, GR\_PUSHER\_GEMPIC)}
    behind a feature flag.
  \item Force evaluation: $\nabla(\vv{v}\cdot\vv{A})$ gather instead
    of $-\partial_t\vv{A}$ and $\vv{v}\times\vv{B}$.
  \item Output / diagnostics: KE and velocity must still be reported
    from $\vv{v}$, computed from $\vv{p}_c$ via
    Eq.~(\ref{eq:gempic_pc}) with the current $\vv{A}$.
\end{itemize}

\subsection{Feature flag and rollout}

GEMPIC lands behind a default-OFF feature flag.  The default scheme
remains the current $\vv{p}_{\mathrm{kin}}$ Boris pusher until the
benchmark suite (§\ref{sec:gempic_benchmark}) confirms GEMPIC passes
the necessary and sufficient acceptance conditions.  When GEMPIC
passes, default flips and the old path becomes a legacy comparison
mode (kept indefinitely for regression).

\subsection{Open items before implementation begins}

\begin{enumerate}
  \item Lock the time-staggering and the $\vv{v}$ implicit-solve
    convention via literature check.  This will be its own
    follow-up scope document (gr\_sandbox\_v37 or similar) once
    benchmark baselines are in hand.
  \item Confirm with a pencil-and-paper derivation that GEMPIC's
    discretisation gives \emph{exactly zero} force on a uniformly-
    moving deposit, at least in the continuum limit and ideally on
    the discrete grid.  This is the analytic check that should
    precede the numerical acceptance run.
  \item Identify whether the gravity-side
    $-m\partial_t \vv{A}_g$ inductive piece has the same structural
    issue and whether the analogous canonical-momentum reformulation
    applies (carry over with $q \to m$, $\vv{A} \to \vv{A}_g$).
  \item Decide whether legacy and GEMPIC pushers share particle
    state (with a flag determining interpretation) or have
    parallel storage.  Storage-flag is simpler; parallel storage
    is more robust against an interpretation-flag bug.  Default
    recommendation: storage-flag, with a runtime assertion that
    the flag matches the pusher selection.
\end{enumerate}
