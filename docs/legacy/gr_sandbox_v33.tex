...
\fancyhead[R]{\small\color{gray}Design Document\,---\,v33}
...
\begin{titlepage}
  ...
  \vspace{1.2cm}
  {\large\bfseries\color{midblue} Revision v33\par}
  \vspace{0.2cm}
  {\small\color{gray} Adds sampled-background stage; clarifies §\ref{sec:abc} realized reflection.\par}
  \vspace{1.5cm}
  \hrule
\end{titlepage}
...
\end{equation}
per orbit, where $a$ is the semi-major axis and $e$ the eccentricity.  This directly verifies that the $(v^2/c^2)\vv{g}$ and $4(\vv{v}\cdot\vv{g})\vv{v}/c^2$ terms are correctly implemented.  See the Stage~6 caveat on discretization noise: if the measured precession is contaminated by the smoothing-length or finite-difference error of the sampled background, fall back to an analytic-background evaluator for this test only.
...
\begin{itemize}
  ...
  \item The orbit period should be unchanged from Stage~8 to within the radiation reaction correction (which is small for weak coupling $G_{\mathrm{eff}} \ll c_{\mathrm{eff}}^2 r / m$).
\end{itemize}
