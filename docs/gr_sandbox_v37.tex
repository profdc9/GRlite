...
\begin{titlepage}
  ...
  detail: Hamilton's equations from the canonical Hamiltonian,
  identification of the discrete operators needed (and what already
  exists in the code), a concrete leapfrog-style scheme proposal, and
  an analytic verification that the proposed discrete force law gives
  zero net force on a uniformly-moving deposit by construction.}
  See §\ref{sec:gempic_derivation}.
  Inherited from v36:
...
high-level scope) carries forward unchanged.  No code changes are
proposed in v37 -- this is the design document that should precede
the GEMPIC implementation per the v36
sec.~\ref{sec:gempic_benchmark} ``Implementation order''.
