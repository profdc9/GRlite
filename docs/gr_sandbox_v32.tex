...
\fancyhead[R]{\small\color{gray}Design Document}
...
Expanding the Christoffel contraction $-\Gamma^\mu{}_{\alpha\beta}U^\alpha U^\beta$ in the linearized metric and collecting by order in $v/c$:
\begin{equation}
\label{eq:geodesic_expansion}
  a^i = \underbrace{g^i}_{\order{1}}
      + \underbrace{4\varepsilon^{ijk}v_j B_{g,k}}_{\order{v/c}}
      + \underbrace{\frac{v^2}{c^2}g^i + \frac{4(\vv{v}\cdot\vv{g})v^i}{c^2}}_{\order{v^2/c^2}}
      + \order{v^3/c^3},
\end{equation}
where $g^i = -\partial_i\Phig - \partial_t A_{g,i}$ and $B_{g,k} = (\nabla\times\Ag)_k$ are evaluated on-the-fly from the potentials.  In implementation the cross-product $\varepsilon^{ijk}v_j B_{g,k}$ is replaced by its potential form $\partial_i(\vv{v}\cdot\Ag) - (\vv{v}\cdot\nabla)A_{g,i}$ via the vector identity, avoiding explicit computation of $\vv{B}_g$ as a grid array (Section~\ref{sec:force_potential}).

The $\order{v^2/c^2}$ terms arise from the spatial metric coefficient $(1-2\Phig/c^2)$:
\begin{itemize}
  \item $(v^2/c^2)\vv{g}$: a fast particle experiences stronger gravitational deflection.  At $v=c$ this doubles the Newtonian deflection, recovering the factor-of-2 result for photons.
  \item $4(\vv{v}\cdot\vv{g})\vv{v}/c^2$: a force along the direction of motion, responsible for perihelion precession.
\end{itemize}
...
\toprule
\textbf{Tier} & \textbf{Gravitational force expression} & \textbf{Valid range} \\
\midrule
0: Newtonian & $-m\nabla\Phig$ & $v/c < 0.01$ \\
1: GEM       & $m(-\nabla\Phig - \partial_t\Ag + 4\nabla(\vv{v}\cdot\Ag) - 4(\vv{v}\cdot\nabla)\Ag)$ & $v/c < 0.1$ \\
2: GEM + $\order{v^2/c^2}$ & Add $(v^2/c^2)\vv{g}$ and $4(\vv{v}\cdot\vv{g})\vv{v}/c^2$ & $v/c < 0.5$ \\
3: Full corrections & All $\order{v^2/c^2}$ terms & $v/c \leq 1$ \\
\bottomrule
\end{tabular}
\end{center}
...
\midrule
Gravitational time dilation & $(1+2\Phig/c^2)$ & Proper time update \\
...
Perihelion precession & Spatial metric & $4(\vv{v}\cdot\vv{g})\vv{v}/c^2$ term \\
\bottomrule
\end{tabular}
\end{center}
...
\end{equation}
For a rotating background the off-diagonal metric components $h_{0i} = 2A_{g,i}/c$ (Section~\ref{sec:gem_metric}) contribute an additional term via $g_{0i}\dd x^i\dd t = (2A_{g,i}/c)v^i\dd t^2$.  The full linearized expression is:
\begin{equation}
\label{eq:proper_time_full}
  \frac{\dd\tau}{\dd t} = \sqrt{\left(1+\frac{2\Phig}{c^2}\right) - \left(1-\frac{2\Phig}{c^2}\right)\frac{v^2}{c^2} + \frac{4\Ag\cdot\vv{v}}{c^2}}.
\end{equation}
The $4\Ag\cdot\vv{v}/c^2$ term has opposite sign for prograde and retrograde orbits around a spinning mass (since $\vv{v}$ reverses but $\Ag$ does not), producing a proper time difference between co- and counter-rotating clocks at the same orbital radius --- the gravitomagnetic clock effect.  For a non-rotating background $\Ag = 0$ and Eq.~(\ref{eq:proper_time_full}) reduces to the simpler form.

To first order:
\begin{equation}
  \frac{\dd\tau}{\dd t} \approx 1 + \underbrace{\frac{\Phig}{c^2}}_{\text{gravitational}} - \underbrace{\frac{v^2}{2c^2}}_{\text{kinematic}} + \underbrace{\frac{2\Ag\cdot\vv{v}}{c^2}}_{\text{gravitomagnetic}}.
\end{equation}
...
The gravitomagnetic term $2\Ag\cdot\vv{v}/c^2$ is nonzero only around rotating masses.  It has opposite sign for co- and counter-rotating orbits because $\vv{v}$ reverses direction while $\Ag$ does not.  Integrating around a full orbit the difference in accumulated proper time between prograde ($+$) and retrograde ($-$) orbits is:
\begin{equation}
  \Delta\tau = \tau_+ - \tau_- = \frac{4}{c^2}\oint\Ag\cdot\dd\vv{x} = \frac{4}{c^2}\int(\nabla\times\Ag)\cdot\dd\vv{A} = \frac{4}{c^2}\int\vv{B}_g\cdot\dd\vv{A},
\end{equation}
by Stokes' theorem --- the proper time difference is proportional to the gravitomagnetic flux through the orbit.  This is the gravitomagnetic clock effect, and it is a direct consequence of the same $\Ag$ that appears in the GEM force expression.
...
\begin{equation}
  \dd\tau_i = \dd t\,\sqrt{1 + \frac{2\Phig}{c^2} - \left(1-\frac{2\Phig}{c^2}\right)\beta^2 + \frac{4\Ag\cdot\vv{v}}{c^2}},
...
The potentials are discretized on a staggered Yee grid, which assigns different components to different spatial locations within each grid cell.  The staggering ensures that finite-difference gradient and curl operations on the potentials are exact --- no interpolation is needed.
...
\end{equation}
The \textbf{adjoint condition} requires $\tilde{W} = W$---the same kernel is used for both operations.
...
The recommended approach for the sandbox is a \textbf{damping layer}: a border region of $N_d$ cells on each side of the grid where the field is multiplied by a spatially varying attenuation factor at each timestep.  Outgoing waves entering the layer are attenuated gradually and the small residual that reaches the hard boundary is attenuated again on the return trip, making double-pass reflections negligible.
...
\textbf{Test:} the Gaussian pulse from Stage~1 should exit the domain cleanly.  Record the residual field amplitude at the domain center after the pulse should have fully exited.  Verify the residual is at the predicted level $R \approx 10^{-3}$ for the chosen $\sigma_{\max}$ and $N_d$.
...
\subsection{Stage 6: Single test particle, Boris pusher, Keplerian orbit}

Add a single test particle with mass $m$ and no charge, orbiting a fixed analytic background $\Phig^{\mathrm{bg}}$ (Schwarzschild-like, no perturbation FDTD).  Implement the potential-form force (Eq.~\ref{eq:force_gem_pot}) and the Boris pusher.

\textbf{Test:} a circular orbit should have period $T = 2\pi r^{3/2}/\sqrt{G_{\mathrm{eff}}M}$.  Verify the total energy $\gamma mc^2 + m\Phig^{\mathrm{bg}}$ is approximately conserved over many orbits with no secular drift (Boris symplectic structure).

\subsection{Stage 7: Relativistic corrections, perihelion precession}

Enable Tier~2 and Tier~3 force corrections (Section~\ref{sec:relcorr}).

\textbf{Test:} a slightly eccentric orbit should precess at the GR rate:
\begin{equation}
  \Delta\phi_{\mathrm{prec}} = \frac{6\pi G_{\mathrm{eff}} M}{c_{\mathrm{eff}}^2\,a(1-e^2)}
\end{equation}
per orbit, where $a$ is the semi-major axis and $e$ the eccentricity.  This directly verifies that the $(v^2/c^2)\vv{g}$ and $4(\vv{v}\cdot\vv{g})\vv{v}/c^2$ terms are correctly implemented.

\subsection{Stage 8: Proper time accumulation}

Add the proper time clock to each particle.

\textbf{Test:} a particle on a circular orbit at radius $r$ should accumulate proper time at rate:
\begin{equation}
  \frac{\dd\tau}{\dd t} = \sqrt{1 + \frac{2\Phig^{\mathrm{bg}}}{c^2} - \frac{v^2}{c^2}}.
\end{equation}
Compare the accumulated $\tau$ after one orbit against the analytic prediction.  This tests both the gravitational and kinematic time dilation contributions simultaneously.

\subsection{Stage 9: Perturbation FDTD with moving particle source}

Enable the full perturbation FDTD: the particle deposits $\rho_{\mathrm{matter}}$ and $\vv{J}_{\mathrm{matter}}$ onto the grid, the wave equations evolve the perturbation potentials, and the particle feels both background and perturbation forces.

\textbf{Test:}
\begin{itemize}
  \item A particle moving at constant velocity in empty space should produce a perturbation field consistent with the boosted Coulomb solution.
  \item A particle in a circular orbit should radiate outgoing gravitational waves visible in $\Phig^{\mathrm{pert}}$.
  \item Outgoing wave amplitude should fall off as $1/\sqrt{r}$ in 2D.
  \item The orbit period should be unchanged from Stage~7 to within the radiation reaction correction (which is small for weak coupling $G_{\mathrm{eff}} \ll c_{\mathrm{eff}}^2 r / m$).
\end{itemize}

\subsection{Stage 10: Two-particle gravitational interaction}
...
\subsection{Stage 11: Charged particles and electromagnetism}
...
\begin{itemize}
  ...
  \item A moving charge in a uniform background magnetic field $B_0$ should undergo circular cyclotron motion at frequency $\omega_c = qB_0/m\gamma$.
  \item The EM vector potential $\vv{A}$ should correctly encode the magnetic field via the Yee curl.
\end{itemize}

\subsection{Stage 12: EM effective GEM sources}
...
\subsection{Stage 13: Intrinsic spin, precession, spin forces}
...
\textbf{Test:}
\begin{itemize}
  \item A gyroscope in the Kerr-like background should precess at rate $\Omega_{\mathrm{LT}} = 2G_{\mathrm{eff}}J/(c_{\mathrm{eff}}^2 r^3)$ for a circular orbit at radius $r$.
...
\subsection{Stage 14: Background spacetime modes}

Add the analytic background spacetimes: Schwarzschild-like, Kerr-like, Reissner-Nordström-like, Kerr-Newman-like, uniform field, binary (Section~\ref{sec:bg}).
...
\subsection{Stage 15: Visualization pipeline}
...
\begin{itemize}
  ...
  \item On-demand field computation ($\vv{g}$, $B_{g,z}$, $\vv{E}$, $B_z$ from potentials) matches analytic values for the Stage~6--14 test configurations.
...
\subsection{Stage 16: Full integration benchmarks}
