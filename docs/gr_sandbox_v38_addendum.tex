...
\subsubsection*{Boundary taper}

If the outer wall enforces zero-Dirichlet (the v38 convention, kept
here for reasons discussed in \S\ref{sec:dc_mode}), the L-W field at
the wall is nonzero and inconsistent with the BC.  Multiply the L-W
field through the absorbing zone by a smooth taper that brings it to
zero at the wall:
\begin{equation}
  \Phi(\vv{x}) \;\leftarrow\; T(d(\vv{x}))\,\Phi(\vv{x}),
  \qquad
  T(d) \;=\;
  \begin{cases}
    1 & d \ge N_\text{damp} \\
    1 - u^2,\; u = (N_\text{damp} - d)/N_\text{damp} & 0 < d < N_\text{damp} \\
    0 & d \le 0
  \end{cases}
\end{equation}
where $d(\vv{x})$ is the cell-count distance from the nearest wall.
This matches the polynomial $m=2$ damping profile of \S 9.6.  Under
Neumann BC the taper must be \emph{omitted}; see \S\ref{sec:dc_mode}.
...
floating-point array \texttt{friction\_d[k]}.  A practical profile is
a small uniform floor plus a polynomial ramp to a larger value at
the wall:
\begin{equation}
  \beta_k \;=\;
  \begin{cases}
  ...
  \end{cases}
\end{equation}
with $d_k$ the cell-count distance from the nearest wall.  Sensible
values for the web demo on a $384 \times 384$ grid with absorber
depth $N_\text{damp} = 48$:
\begin{equation}
  \beta_\text{floor} \;=\; 10^{-3},
  \qquad
  \beta_\text{wall}  \;=\; 2 \times 10^{-2}.
\end{equation}
Mode lifetimes: lowest standing mode $\sim 1000$ steps; modes
localized within the ring $\sim 50$ steps.  At
CFL $= 1/\sqrt{2}$ this is well inside the stability region for both
the bulk and the wall.
...
\begin{algsteps}
  ...
  \item Evaluate Eqs.~(\ref{eq:phig_lw})-(\ref{eq:A_lw}) at every grid
    cell for every particle.  Clip $r$ at $\half\dd x$ to avoid the
    log singularity at the source.
  \item If the outer BC is Dirichlet, multiply through by the
    polynomial-$m=2$ taper that brings the field smoothly to zero at
    the wall over the depth of the absorbing zone.  If Neumann, do
    not taper -- the discontinuity would launch an inward wave on
    step 1.
  \item Copy the resulting curr buffer into prev and next so the
    encoded time derivative is identically zero.
  \item Begin the wave-equation evolution.  No half-step momentum
    back-extrapolation is needed because the field is already at the
    static fixed point.
\end{algsteps}
...
\begin{algsteps}
  ...
  \item Populate with a tapered profile: $\beta_\text{floor}$ in the
    interior, smoothly increasing to $\beta_\text{wall}$ over the
    last $N_\text{damp}$ cells from each wall.
...
\paragraph{What works.}  The friction absorber + Dirichlet + L-W
initialization combination reaches a stable post-transient
configuration on both pic\_static (single mass) and pic\_binary (two
orbiting masses):
\begin{itemize}[leftmargin=*,nosep]
  \item Initial Dirichlet-induced inward wave from the L-W taper at
    the wall damps within a few hundred steps.
  \item No lowest-mode trap; no inversion oscillation.
  \item The bound binary keeps orbiting after the transient.
  \item Discrete-Dirichlet-Poisson static is reached and held.
\end{itemize}

\paragraph{Open follow-ups.}  Tracked separately:
\begin{itemize}[leftmargin=*,nosep]
  \item \emph{COM drift on pic\_binary.}  After the boundary transient
    damps, the two-body center of mass moves at small constant
    velocity.  Suspected cause: the transient imparts a momentum
    impulse to the pair that is not exactly equal and opposite due
    to sub-cell grid alignment of the particles.  Fix is to make the
    L-W IC consistent with the discrete Dirichlet equilibrium so no
    transient is launched, either via TSC-convolved L-W or via a few
    SOR sweeps applied to the L-W as a smoother.
  \item \emph{Continuum-discrete Poisson residual.}  See
    \S\ref{sec:lw_discrete}.  Currently dissipated by friction; could
    be eliminated by TSC-convolved L-W.
  \item \emph{Neumann + zero-mean momentum bleed.}  See
    \S\ref{sec:dc_mode}, item 1.  Currently worked around by using
    Dirichlet.
\end{itemize}
