...
no-inductive drift is larger and slower-to-converge than gravity's
because the EM scalar gradient still uses CIC$+$LEGACY (TSC$+$LB
variants of \texttt{phi\_em\_grad\_at\_total} are not yet ported from
the gravity side).  The inductive contribution itself is CFL-converging
on schedule.
...
\paragraph{Sign convention.}  In this simulator's 2D-log Coulomb
(PIC perturbation channel), \emph{same-sign} charges attract --- the
direct analog of 2D-log gravity where positive masses attract via
$\Phig = 2 G m \ln r$.  The Green's function of the 2D Laplacian
flips the gradient direction relative to the 3D $1/r$ form.  The
analytic $1/r$-shape point-charge background
(\texttt{gr\_sim\_set\_background\_point\_charge}) is the deliberate
exception: it superimposes a 3D-style 1/r potential on top of the 2D
PDE so test-particle Kepler stages (Stage~26, Stage~27) recover the
familiar ``opposite charges attract'' picture.  The PIC perturbation
always carries the 2D-log sign, so for an attractive binary we use
$q_1 = q_2 = +Q$.
...
\begin{itemize}
  \item \emph{Free-fall:} two like charges released at rest fall together
        from $r = 8$ to $r_{\min} = 0.077$ ($1\%$ of $r_{\mathrm{init}}$,
        essentially head-on contact) within 100 steps, then bounce out
        with only $\sim 6\%$ excess radius ($r_{\max} = 8.46$) and
        oscillate.  Mutual attraction engages with the correct sign.
        Esirkepov violations $= 0$ throughout.
  \item \emph{Short-horizon orbit:} at $r = 8$, $v_{\mathrm{orb}} = 0.1$,
        the empirical $\omega$ matches the analytic $v_{\mathrm{orb}}/r$
        to $0.4\%$ at $s = 20$ (one wave-crossing time after start).
        Radial drift at $s = 20$ is $1.6\%$.
  \item \emph{$v_{\mathrm{orb}}$ independent of $r$:} at $r = 6$ and
        $r = 12$, $v_{\mathrm{meas}} = r \omega$ values are
        $0.1027$ and $0.1031$ ---  matching each other to $0.4\%$ and
        the analytic $Q\sqrt{k_e/m} = 0.1$ to $\sim 3\%$.  Confirms the
        2D-log signature on the EM channel.
\end{itemize}
