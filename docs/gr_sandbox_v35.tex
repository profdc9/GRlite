...
\paragraph{Status of the EM Lorentz force after Stages~9e and 9f.}
\begin{center}
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Piece of $\vv{F}_{\mathrm{em}}$} & \textbf{Implemented} & \textbf{Verified by} & \textbf{Bg evaluator} \\
\midrule
$+ q\,\vv{v}\!\times\!\vv{B}$ (magnetic Lorentz) & yes & Stage 9e & \texttt{gr\_bg\_eval\_B\_em} \\
$- q\nabla\phi$ (electrostatic)                  & yes & Stage 9f & \texttt{gr\_bg\_eval\_phi\_em} \\
$- q\,\partial_t\vv{A}$ (inductive)              & not yet & ---  & --- \\
\bottomrule
\end{tabular}
\end{center}
The inductive $-q\,\partial_t\vv{A}$ piece requires sampling a one-sided
time derivative of the $\vv{A}$ arrays in the pusher ($(\vv{A}_{\mathrm{curr}}
- \vv{A}_{\mathrm{prev}})/\Delta t$, evaluated at the same half-step
time as the magnetic curl).  For a static or slowly-varying
$\vv{A}^{\mathrm{bg}}$ this piece is negligible; it will be wired in
with the radiative-EM verification stage when a time-varying
$\vv{A}^{\mathrm{pert}}$ becomes the primary observable.
