...
PIC, all three EM Lorentz pieces                & $\mathbf{+18.6\%}$ & PIC heating dominates \\
...
\begin{itemize}
  ...
  \item Gravity (Stage 18b at $m_{\mathrm{test}} = 10^{-3}$) is
    stable in part because it does \emph{not} implement an analogous
    $-\partial_t\vv{A}_g$ piece (the doc's Tier-3 form
    Eq.~\ref{eq:eih_full} includes it, but the implementation predates
    Stage~9g and so has only the spatial-derivative GEM feedback,
    where the coefficient~$+4$ on $\vv{v}\!\times\!\vv{B}_g$ gives a
    strong stabilising counter-torque).
  \item Disabling the inductive piece via
    \texttt{gr\_sim\_set\_em\_inductive\_enabled(sim, 0)} is the
    current production recommendation for closed-loop EM PIC, until
    a better discretization is found.
  \item Alternative discretizations to investigate: the
    backward difference $(\vv{A}^n - \vv{A}^{n-1})/\Delta t$ at
    $t^{n-1/2}$ (which would avoid reading $\vv{A}^{n+1}$ at the cost of
    a half-step time shift), or implicit/symplectic time integration
