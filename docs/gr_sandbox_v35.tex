...
\begin{titlepage}
  ...
  \vspace{0.2cm}
  {\small\color{gray}
  \textbf{Major architectural pivot}: migrates the sandbox from a
  cell-centered grid to the proper Yee staggered grid, enabling
  Esirkepov-grade charge-conserving current deposition.  Motivation
  documented in the new Section~\ref{sec:yee_pivot}: at the cell-centered
  layout with CIC deposition, the numerical PIC ``grid-heating''
  self-force scales like the physical mutual-gravity force on a
  test particle in mutual orbit, so quantitative dynamic-field PIC
  is fundamentally limited.  Stages 10 and 11 in v34 worked only in
  the weak-coupling test-particle regime; with Yee$+$Esirkepov the
  Hockney--Eastwood adjoint cancellation becomes exact \emph{and}
  preserved under particle motion, unlocking quantitative two-body
  gravitational dynamics (Stage~11+) and quantitative gravitational
  radiation (later stages).\par
  Section~\ref{sec:yee_pivot} also includes the migration plan with
  save points S1--S7; intermediate cell-centered version preserved on
  the \texttt{v34-cell-centered} git branch.\par
  All v34 physics corrections (EIH 1PN sign$+$Shapiro term, proper-time
  Eq.~\ref{eq:proper_time_full}, analytic-background mode) carry forward
  unchanged.\par}
  \vspace{1.5cm}
  \hrule
\end{titlepage}
...
%====================================================================
\section{Architectural Pivot in v35: Cell-Centered to Yee Staggered Grid}
\label{sec:yee_pivot}
%====================================================================

\subsection{Why this pivot is being made}

The v32--v34 design used a \emph{cell-centered} grid: all six potentials,
all six source arrays, and all derived field quantities lived at the
same set of cell-center sample points $(i+\tfrac{1}{2}, j+\tfrac{1}{2})\Delta x$.
Source-deposition followed the standard particle-in-cell (PIC) recipe
with the W$_2$ (cloud-in-cell, CIC) kernel, and force interpolation
used a CIC-of-FD-gradient operator with the same kernel weights to
satisfy the Hockney--Eastwood adjoint condition for self-force
cancellation.

Stages 1--9 of the v34 implementation worked correctly under this
layout, because they each rely on at most a \emph{static} particle in a
\emph{fixed} background.  The Hockney--Eastwood theorem applies and
the self-force on a stationary particle is exactly zero (verified
quantitatively in v34's Stage 10 Test C: drift $< 10^{-5}$ cells over
$10^4$ timesteps).

\paragraph{The breakdown in v34 Stage 10+.}  Once particles \emph{move}
and \emph{deposit dynamic sources}, the adjoint cancellation no longer
applies pointwise.  The residual takes the form of a numerical
``grid-heating'' self-force, with magnitude scaling like the
gradient of the particle's own deposited perturbation field at its
current position.  Crucially, in our cell-centered + CIC setup, the
\emph{sign} of this residual is such that it \emph{adds} energy to the
particle's orbit rather than removing it (the opposite of physical
radiation reaction).  The scaling, in our natural units:
\begin{equation}
...
\paragraph{What this means for the original sandbox ambition.}  The
v32--v34 architecture limits FDTD's role to \emph{visualization} of
fields and to \emph{quantitative tests with analytic backgrounds};
quantitative dynamic-field PIC for mutual gravity is out of reach.
That undermines the original framing of the project (``a dynamic-field
2D linearized GR PIC simulation''): the FDTD is mostly carrying
weight as visualization scaffolding around an analytic backbone.

\subsection{The Yee + Esirkepov fix}

The standard remedy in PIC plasma simulation is the \emph{Yee
staggered grid}~\cite{Yee1966,TafloveHagness2005} paired with
\emph{Esirkepov current deposition}~\cite{Esirkepov2001}.  Together
they enforce \emph{exact discrete charge conservation} on the
discretized grid, which makes the moving-particle self-force residual
identically zero by construction (modulo the underlying linearization
truncation, which is the irreducible part).

\paragraph{Yee lattice layout (2D).}  We adopt the standard Yee
placement, re-mapped to our six-potential GEM$+$EM list:
\begin{center}
\begin{tabular}{@{}L{4.5cm}L{4.5cm}L{4.5cm}@{}}
\toprule
\textbf{Quantity} & \textbf{Lattice node} & \textbf{Index} \\
\midrule
$\Phig$, $\phi_{\mathrm{em}}$, $\rho_{\mathrm{matter}}$, $\rho_q$
  & Cell corners & $(i, j) \cdot \Delta x$ \\
$A_{g,x}$, $A_x$, $J_{m,x}$, $J_{q,x}$
  & Horizontal edge centers & $(i+\tfrac{1}{2}, j) \cdot \Delta x$ \\
$A_{g,y}$, $A_y$, $J_{m,y}$, $J_{q,y}$
  & Vertical edge centers & $(i, j+\tfrac{1}{2}) \cdot \Delta x$ \\
$B_{g,z}$, $B_z$ (derived, not stored)
  & Cell centers & $(i+\tfrac{1}{2}, j+\tfrac{1}{2}) \cdot \Delta x$ \\
\bottomrule
\end{tabular}
\end{center}

In storage, each of the 12 grid arrays continues to be an $N_x \times
N_y$ float array; \emph{their conceptual lattice positions differ}.
The interpretation of array index $(i,j)$ depends on which field it
belongs to, per the table above.

\paragraph{Natural Yee operators.}  Several finite-difference
operations become exact on the staggered lattice without
interpolation:
\begin{align}
  \text{Gradient of scalar}: && (\partial_x \Phig)_{i+1/2,j}
    &= \frac{\Phig_{i+1,j} - \Phig_{i,j}}{\Delta x}, \\
  \text{Yee curl (in 2D)}: && (B_z)_{i+1/2,j+1/2}
    &= \frac{A_{y,i+1,j+1/2} - A_{y,i,j+1/2}}{\Delta x}
    - \frac{A_{x,i+1/2,j+1} - A_{x,i+1/2,j}}{\Delta x}, \\
  \text{Vector-potential time deriv}: && (\partial_t A_x)_{i+1/2,j}
    &= \frac{A_{x,i+1/2,j}^{n+1} - A_{x,i+1/2,j}^{n-1}}{2\Delta t}.
\end{align}
Note that $\partial_x \Phig$ at $(i+\tfrac{1}{2}, j)$ lives on the
\emph{same} sublattice as $A_x$, so the local gravity vector
$g_x = -\partial_x \Phig - \partial_t A_x$ is computable with no
interpolation.  This is the architectural advantage of Yee for
field theories: derivative operators preserve sublattice membership.

\paragraph{Wave-equation leapfrog.}  The leapfrog update remains
\begin{equation}
  \Phi^{n+1} = 2\Phi^n - \Phi^{n-1} + (c\,\dd t)^2 \left( \nabla^2_h \Phi^n + s_{\mathrm{coeff}} \cdot \mathrm{source}^n \right),
\end{equation}
applied identically to each of the six potentials.  The only change is
that each field's discrete Laplacian uses its own sublattice's
neighbors, and the boundary-cell indexing is shifted by half a cell
for the edge-component fields.

\paragraph{Esirkepov current deposition.}  Given a particle's positions
$\vv{x}^n$ and $\vv{x}^{n+1}$, Esirkepov's recurrence directly computes
$J_x$ at the relevant horizontal-edge cells and $J_y$ at the relevant
vertical-edge cells such that the \emph{exact discrete continuity
equation}
\begin{equation}
\label{eq:discrete_continuity_yee}
  ...
  + \frac{J_y^{n+1/2}_{i,j+1/2} - J_y^{n+1/2}_{i,j-1/2}}{\Delta x}
  \;=\; 0
\end{equation}
holds at every corner $(i,j)$.  The recurrence is local (involves
only the cells touched by the particle's trajectory across one
timestep) and is $\sim 50$ lines in 2D.  Once $\rho$ and $\vv{J}$
satisfy Eq.~(\ref{eq:discrete_continuity_yee}) exactly, the
Hockney--Eastwood-style residual self-force vanishes by construction
under any rigid motion of the particle.
