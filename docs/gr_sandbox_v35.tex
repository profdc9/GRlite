...
\end{itemize}
\paragraph{Open item.}  The post-fix inductive contribution at the
same CFL sweep reads $+3.5\%,\ +23.4\%,\ +25.4\%,\ +22.3\%$ (drift,
not contribution-only) --- an \emph{outspiral} that is also
CFL-converged (ratio approaching $1.0$).  Pre-v36 the same setup
gave inspiral; the sign flip on the field convention also flips the
sign of the inductive-piece self-coupling, so the discrete-PDE
radiation-reaction direction reversed.  The energy-conservation
argument says outgoing waves must drain orbital energy (inspiral),
so the outspiral indicates a residual time-staggering mismatch in
the inductive piece that the v36 sign flip exposed.  Tracking down
the source of this mismatch is the next priority for the EM channel.
