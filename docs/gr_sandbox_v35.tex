...
on a probe charge}) is now end-to-end functional on the force-evaluation
side; see Section~\ref{sec:stage_em_pic_orbit} for the closed-loop
...
\paragraph{Implications and open questions.}
\begin{itemize}
  \item The EM Lorentz force is correctly wired \emph{per piece}
    (Stages 9e/9f/9g all pass in isolation).  The instability is a
    \emph{closed-loop} PIC phenomenon, not a coefficient or sign bug.
  \item Gravity (Stage 18b at $m_{\mathrm{test}} = 10^{-3}$) appears
    stable, but \emph{the gravity-side implementation does not include
    an analogous $-m\,\partial_t\vv{A}_g$ piece}.  The doc's Tier-3
    form (Eq.~\ref{eq:eih_full}, Section~\ref{sec:alg_rel} eqbox at
    line 1040) does include it.  Stage 28 (added 2026-05-21 as a
    diagnostic at the user's request) wires the GEM inductive piece
    using the same centered-difference structure and reruns pic\_orbiting
    at $m_{\mathrm{test}} = 10^{-3}$: gravity exhibits \textbf{essentially
    identical heating} ($+23\%/4\,\mathrm{orbits}$ outward, even worse
    than EM's $+18.6\%$).  The gravity ``stability'' of Stage 18b is
    therefore an accidental consequence of omitting the inductive piece;
    the issue is \emph{not} EM-specific.

  \item \textbf{Sign-flip diagnostic.}  At the user's suggestion the
    sign multiplier on $\partial_t\vv{A}$ was made runtime-tunable
    (default $+1.0$; setting $-1.0$ flips the sign).  Both EM and gravity
    show mirror-image behaviour: $-1$ produces $\sim-25\%$ inward drift,
    $+1$ produces $\sim+25\%$ outward drift, both accelerating
    orbit-by-orbit.  This RULES OUT a sign-convention bug --- if the sign
    were just wrong, flipping should recover the small ($\sim 0.1\%/$
    orbit) radiation-reaction inspiral predicted by the Larmor formula
    at this coupling.  Instead, the flipped sign gives the
    opposite-direction divergence at the same magnitude.  The
    centered-time-difference inductive term injects a sign-determined
    energy pump whose magnitude is $3$--$4\times$ larger than physical
    radiation reaction at this coupling.  Whichever sign is chosen, the
    discretization-induced effect dominates.
  \item Two simple-difference discretizations of $\partial_t\vv{A}_{\mathrm{em}}$
    were tried in Stage~27 and both heat:
    \begin{itemize}
      \item Centered $(\vv{A}^{n+1} - \vv{A}^{n-1})/(2\,\Delta t)$
        at $t^n$: $+18.6\%$ over 4 orbits (heating dominates inspiral).
      \item Backward $(\vv{A}^n - \vv{A}^{n-1})/\Delta t$
        at $t^{n-1/2}$: unbound by orbit 4 ($+5\%, +22\%, +47\%, \infty$).
        Initially seemed analogous to the read-prev fix for the
        spatial gradient (both slices are pre-current-step), but the
        SPATIAL-time mismatch (reading the field's past values at the
        particle's \emph{current} position) injects even more energy
        than the centered diff's self-deposit inclusion.
    \end{itemize}
  \item Disabling the inductive piece entirely via
    \texttt{gr\_sim\_set\_em\_inductive\_enabled(sim, 0)} is the
    current production recommendation for closed-loop EM PIC.
  \item The proper fix is likely \emph{implicit / symplectic time
    integration of the combined particle + field system}, where the
    field $\vv{A}^{n+1}$ at the new step is solved consistently with
    the particle's new position $\vv{x}^{n+1}$, preserving discrete
    energy conservation exactly.  This is a substantial restructuring
    of the gr\_sim\_step pipeline and is left as future work.
\end{itemize}

The Stage 27 test passes if at least one of the two PIC variants stays
bound; current production behaviour (inductive on) is bound but
heating, and the "honest" radiation-reaction inspiral is recovered
when the inductive gate is off.
