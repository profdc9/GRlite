...
$\order{v/c}$ Lense--Thirring force piece; the orbit-shape Lense--Thirring
precession around a \emph{spinning point mass} (where $\vv{B}_g$ varies
spatially as $(\nabla\!\times\!\Ag)_z$ for the dipole) is a separate
verification.  Together with Section~\ref{sec:stage_proper_time} Phase~B
(clock effect) and a planned Lense--Thirring precession stage
(Section~\ref{sec:stage_proper_time}'s spinning background with the GM
force enabled), they exhaust the leading-order $\order{J_z}$ tests of
the GEM coupling.
